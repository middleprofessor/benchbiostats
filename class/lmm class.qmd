...
```{r, class.source = "fold-show", eval=FALSE}
pptt <- aov_4(y ~ treatment + (1 | block),
             data = figx)
```
...
4. aov2 -- multivariate RM-ANOVA
5. aov1 -- univariate RM-ANOVA
6. pptt -- pairwise, paired t-test

(Cannot use lmm3 or lmm4 because there is no replication of each block:treatment combination.)

for a 2 x 2
7. lmm4 -- random interaction intercep model with each factor:block intercept


Assumptions:

1. lmm1, lm1, and aov1 assume
    * **compound symmetry**.
    * **sphericity**.
2. lmm2, aov2, and pptt do not assume compound symmetry and sphericity.

Equivalent models if design is balanced

1. single factor models

2. two crossed factor models
* lm1, aov1_flat, and lmm1 are equivalent
* lmm4 and oav1 are equivalent
* aov2 and pptt are equivalent
* lmm2 is equivalent to aov2/pptt in SE and t but df differ so p differs

Notes:

1. lmm1 is the standard. If the number of treatments = 2, then lmm1 is eqivalent with a paired t-test.
2. If the design is balanced AND the number of treatments = 2 then *all six methods are numerically equivalent*.
3. If the design is balanced AND the number of treatments > 2, then
    * lmm1, lm1, and aov1 are numerically equivalent
    * lmm2, aov2, pptt result in the same estimates and SE but lmm2 has more df, so the CIs, and p-values of lmm2 are less conservative.
4. If the number of treatments = 2 AND the design is not balanced (at least one block is missing value for one treatment level) then the linear models (lm1, aov2, aov1, pptt) are equivalent but the linear mixed models (lmm1, lmm2) differ from each other and from the linear models.
...
6. If a treatment within a block is missing, the block is deleted only in the comparisons including the missing treatment in the pairwise, paired t-tests. This makes pptt more powerful than aov2.
...
1. Fit the most complete model first, if the model fails or has boundary warning because of too little variance at some level, then fit the next most complete model, and so on. This order would be lmm2, lmm1, lm1 (lmm1 and lm1 are equivalent if the design is balanced).
2. Fit all linear mixed models (lmm2, lmm1) and use a model selection method to choose the best model that balances completeness (controling type I error) and overfitting.
...
error_type <- "type1" 
if(error_type == "type1"){
  outfile_name <- "rcbd_type1-2.Rds"
  # parameters
  beta_i = c(10, 0, 0)
}
if(error_type == "power"){
    outfile_name <- "rcbd_power-2.Rds"
  # parameters
  beta_i = c(10, 0, 1.2)
}
...
model_levels <- c("lm0", "lm1", "aov2", "aov1", "pptt", "lmm2")
  ...
  for(param_set in 1:nrow(param_set_matrix)){
    ...
    for(i in 1:n_sims){
      ...
      form_lm1 <- paste(paste0("sim_", i), "~ treatment + experiment") |>
        formula()
      form_lmm1 <- paste(paste0("sim_", i), "~ treatment + (1 | experiment)") |>
        formula()
      ...
      # lmm1 <- lmer(form_lmm1, data = fake_rcbd)
      lmm2 <- lme(form_lm0,
                  random = ~1 | experiment,
      ...
      # aov1 used for both multivariate (aov2) and univariate (aov1)
      aov1 <- aov_4(form_aov,
                    fun_aggregate = mean,
                    include_aov = TRUE,
                    data = fake_rcbd)
      ...
      lm1_pairs <- emmeans(lm1, specs = "treatment") |>
        contrast(method = "revpairwise", adjust = "none")
      aov2_pairs <- emmeans(aov1, specs = "treatment", model = "multivariate") |>
        contrast(method = "revpairwise", adjust = "none")
      aov1_pairs <- emmeans(aov1, specs = "treatment", model = "univariate") |>
        contrast(method = "revpairwise", adjust = "none")
...
2. When there is zero correlated error, of the methods that model non-independence, all have reasonable type I error control. Perhaps lmm2 is very slightly anticonservative (one of the error rates is 0.07).
3. When there is equal correlated error, of the methods that model non-independence, all have reasonable type I error control (lmm2 is a very slightly anti-conservative).
4. When there is heterogenous correlated error, of the methods that model non-independence, only aov2/pptt/lmm2 have reasonable type I error control (again, lmm2 is very slightly anticonservative). lmm1/aov2 have liberal Type I control in contrasts with low correlated error and conservative Type I control in contrasts with high correlated error.

Combined, these results suggest that aov2/pptt should be our preferred model, if we only take Type I control into consideration. But we should definitely consider Power.
...
1. The naive linear model (lm1) has less power than methods that model non-independence. lm1 only has about the same power as methods that model non-independence if there is something close to zero correlated error. This is why we want to use models for non-independence when we have a blocked experimental design.

Results to inform our decision about which model for non-independence

2. When there is zero correlated error, of the methods that model non-independence, lmm2 has slightly higher power than lmm1/aov1/lm1. And lmm1/aov1/lm1 have slightly higher power than aov2/pptt.
3. When there is equal correlated error, of the methods that model non-independence, lmm2 and lmm1/aov1/lm1 have slightly higher power than aov2/pptt.
4. When there is heterogenous correlated error, of the methods that model non-independence, lmm1/aov1/lm1 have non-trivially higher power than aov2/pptt in the contrast with the lower correlated error is but non-trivially lower power in the contrast with the higher correlated error. lmm2 has relatively high power regardless of the correlated error of the contrast. The major issue in experimental bench biology is that sample sizes are generally far too low to have good estimates of these correlations or of variance components.

These results suggest lmm2 is the preferred model. If lmm2 fails, then it's a toss-up between aov2/pptt and lmm1/aov1/lmm1.

But, combining the Type I and power simulations, lmm1/aov1/lmm1 have poor type I error control under some conditions while lmm2 and aov2/pptt have good error control under all conditions.
...
model_levels <- c("lm0", "lm1", "lmm4", "aov2", "aov1", "aov1_flat", "pptt", "lmm2")
...
#    fake_rcbd[, c("diet", "drug") := tstrsplit(treatment, "-")]
    ...
    for(i in 1:n_sims){
      ...
      # models
      form_lm0 <- paste(paste0("sim_", i), "~ diet * drug") |>
        formula()
      form_lm1 <- paste(paste0("sim_", i), "~ diet * drug + litter") |>
        formula()
      form_lmm1 <- paste(paste0("sim_", i), "~ diet * drug + (1 | litter)") |>
        formula()
      form_lmm4 <- paste(paste0("sim_", i), "~ diet * drug + (1 | litter) + (1 | litter:diet)  + (1 | litter:drug)") |>
        formula()
      ...
      lmm4 <- lmer(form_lmm4, data = fake_rcbd) |>
        try()
      if(inherits(lmm2, "try-error")){
        lmm4 <- NULL
      }
      ...
      aov1 <- aov_4(form_aov,
                    ...
                    include_aov = TRUE,
                    data = fake_rcbd)
      aov1_flat <- aov_4(form_aov_flat,
                      include_aov = TRUE,
                      data = fake_rcbd)
      
      lm0_pairs <- emmeans(lm0, specs = c("diet", "drug")) |>
        contrast(method = "revpairwise", simple = "each", combine = TRUE, adjust = "none")
      lm1_pairs <- emmeans(lm1, specs = c("diet", "drug")) |>
        contrast(method = "revpairwise", simple = "each", combine = TRUE, adjust = "none")
      lmm1_pairs <- emmeans(lmm1, specs = c("diet", "drug")) |>
        contrast(method = "revpairwise", simple = "each", combine = TRUE, adjust = "none")
      lmm4_pairs <- emmeans(lmm4, specs = c("diet", "drug")) |>
        contrast(method = "revpairwise", simple = "each", combine = TRUE, adjust = "none")
      aov2_pairs <- emmeans(aov1, specs = c("diet", "drug"), model = "multivariate") |>
        contrast(method = "revpairwise", simple = "each", combine = TRUE, adjust = "none")
      aov1_pairs <- emmeans(aov1, specs = c("diet", "drug"), model = "univariate") |>
        contrast(method = "revpairwise", simple = "each", combine = TRUE, adjust = "none")
      aov1_flat_pairs <- emmeans(aov1_flat, specs = c("treatment"), model = "univariate") |>
        contrast(method = "revpairwise", simple = "each", combine = TRUE, adjust = "none")
      ...
      aov2_p <- summary(aov2_pairs)[, "p.value"]
      aov1_p <- summary(aov1_pairs)[, "p.value"]
      aov1_flat_p <- summary(aov1_flat_pairs)[, "p.value"]
      pptt_p <- pptt_pairs[, p.value]
      ...
      p4_mat[i, 3] <- aov2_p[4]
      p4_mat[i, 4] <- aov1_p[4]
      p4_mat[i, 5] <- aov1_flat_p[5]
      p4_mat[i, 6] <- pptt_p[5]
      
      if(!is.null(lmm2)){
        lmm2_pairs <- emmeans(lmm2, specs = c("diet", "drug"), mode = "satterthwaite") |>
          contrast(method = "revpairwise", simple = "each", combine = TRUE, adjust = "none")
        p1_mat[i, 7] <- summary(lmm2_pairs)[1, "p.value"]
        p2_mat[i, 7] <- summary(lmm2_pairs)[2, "p.value"]
        p3_mat[i, 7] <- summary(lmm2_pairs)[3, "p.value"]
        p4_mat[i, 7] <- summary(lmm2_pairs)[4, "p.value"]
      }else{
        p1_mat[i, 7] <- NA
        p2_mat[i, 7] <- NA
        p3_mat[i, 7] <- NA
...
2. When there is zero correlated error, of the methods that model non-independence, all have reasonable type I error control, except lmm2, which is anticonservative.
3. When there is equal correlated error, of the methods that model non-independence, all have reasonable type I error control, except lmm2, which is anticonservative.
4. When there is heterogenous correlated error, of the methods that model non-independence, only aov2/pptt/lmm2 have reasonable type I error control (again, lmm2 is slightly anticonservative). lmm1/aov2 have liberal Type I control in contrasts with low correlated error and conservative Type I control in contrasts with high correlated error.
...
2. If the number of treatments > 2 
    ...
    * if the design is balanced: use aov2/pptt.
...
if(doit == TRUE){
  ...
  for(i in 1:n_sim){
    ...
    # aov2 <- aov_4(y ~ tr1_main * tr2_sub + (tr1_main | block),
    #               data = fake_i)
    # 
...
6. aov2 -- multivariate RM-ANOVA
7. aov1 -- univariate RM-ANOVA
...
3. If design is balanced
    * lmm3 and aov2 result in same estimates, SE, CIs, and p-values
    * lmm4, lmm1, and aov1 result in same estimates, SE, CIs, and p-values
    * lmm2 has same estimates and SE as lmm3/aov2 but more df so less conservative
...
7. lmm1, lmm4, and aov1 will be less conservative (more power but at higher type I error/false positive rate)

Best practices

1. If balanced and no covariates then use lmm3/lmm2/aov2 if discovery is expensive (want to avoid false positives) or lmm4/aov1/lmm1 if discovery is cheap (can afford false positives). lmm2 is less conservative than lmm3/aov2. Use lmm3 if interested in the variance at different levels and/or covariance structure.
...
5. aov2 -- multivariate RM-ANOVA
6. aov1 -- univariate RM-ANOVA
...
### aov2 -- repeated measures ANOVA (RM-ANOVA)

```{r}
aov2 <- aov_4(nr4a1 ~ genotype + (genotype | experiment_id), data = fig2c)
aov2_emm <- emmeans(aov2, specs = "genotype")
aov2_pairs <- contrast(aov2_emm, method = "revpairwise") |>
  summary(infer = TRUE)
aov2_pairs
```
...
3. aov2 fits an ANOVA model even though there are only two groups. Yes ANOVA can be used with only two groups, and the statistics are exactly the same as with a *t*-test, but most people choose *t*-test and not ANOVA when there are only two groups.
...
ggptm(lmm1, lmm1_emm, lmm1_pairs,
              join_blocks = TRUE,
...
lmm1_dt <- data.table(lmm1_pairs)
lmm1_dt[, pretty_p := lazyWeave::pvalString(p.value)]

# now copy the first part of each contrast into group 1 and 2nd part into group 2
lmm1_dt[, group1 := c("TIM3-")]
lmm1_dt[, group2 := c("TIM3+")]
...
gg1 <- ggplot(data = fig2c,
  ...
  geom_line(aes(group = experiment_id),
            position = position_dodge(width = 0.2),
            color = "gray40") +
  geom_point(data = summary(lmm1_emm),
             aes(y = emmean,
                 color = genotype),
             size = 3) +
  geom_errorbar(data = summary(lmm1_emm),
                aes(y = emmean,
                    ymin = lower.CL,
                    ymax = upper.CL,
                    color = genotype),
...
lmm1 <- lmer(pd1_mfi ~ genotype + (1 | experiment_id), data = fig3e)
lmm1_emm <- emmeans(lmm1, specs = "genotype")
lmm1_pairs <- contrast(lmm1_emm, method = "revpairwise") |>
  summary(infer = TRUE)
...
aov2 <- aov_4(pd1_mfi ~ genotype + (genotype | experiment_id), data = fig3e)
aov2_emm <- emmeans(aov2, specs = "genotype")
aov2_pairs <- contrast(aov2_emm, method = "revpairwise") |>
  summary(infer = TRUE)
...
lm0 <- lm(pd1_mfi ~ genotype, data = fig3e)
summary(lm0)$sigma
...
```{r}
lmm1 <- lmer(nr4a1 ~ genotype + (1 | experiment_id), data = fig2c_r)
lmm1_emm <- emmeans(lmm1, specs = "genotype")
lmm1_pairs <- contrast(lmm1_emm, method = "revpairwise", adjust = "none") |>
  summary(infer = TRUE)
lmm1_pairs
```
### lmm2 -- linear model for correlated error

```{r}
# lmm3 <- lme(nr4a1 ~ genotype,
...
### aov2 -- repeated measures ANOVA (RM-ANOVA), multivariate model

```{r}
aov2 <- aov_4(nr4a1 ~ genotype + (genotype | experiment_id), data = fig2c_r)
aov2_emm <- emmeans(aov2, specs = "genotype")
...
### aov1 -- repeated measures ANOVA (RM-ANOVA), univariate model

```{r}
aov1 <- aov_4(nr4a1 ~ genotype + (genotype | experiment_id),
              data = fig2c_r,
              include_aov = TRUE)
aov1_emm <- emmeans(aov1,
                    specs = "genotype",
                    model = "univariate")
...
```{r}
pptt1 <- pptt(nr4a1 ~ genotype + (genotype | experiment_id), data = fig2c_r)
print(pptt1)
```
...
1. lmm2 fails.
2. aov2 and pptt1 are equivalent. This is generally true if there are no missing data (genotype:experiment_id combinations). Both aov2 and pptt1 require balance but pptt1 is a pairwise procedure while aov2 analyzes all treatment levels simultaneously. Consequently pptt1 will exclude fewer measures and will have slightly more power than aov2. 
3. aov2 and lmm1 are not equivalent and this is generally true. 
4. aov1 and lmm1 are equivalent and this is generally true if there are no missing data.
5. The lmm equivalent of aov2 is lmm3 (random intercept and slopte model) but this model cannot be fit without replication with genotype:experiment_id combinations.
6. One reasonable conclusion of item 4 is that aov2 effectively overfits the data and we should use aov1/lmm1 as the model of choice. The simulation under RCBD above argues against this. Instead, aov2/lmm3 is preferred (although lmm3 cannot be fit without replication)
7. Item 5 raises the question, why bother with random intercept model in a RCBD? The random intercept model (lmm1) should be used if there are covariates or if the response type (count, proportion, amount) and model check suggest a generalized linear model (glm).

### Plot the model

```{r lmm-fig2c_r_ggplot-auto, echo=FALSE, eval=FALSE}
plot_response(aov2, aov2_emm, aov2_pairs,
              join_blocks = TRUE,
              block_id = "experiment_id")
```
...
aov2_emm_dt <- aov2_emm |>
  data.frame() |>
  data.table()
aov2_emm_dt[, genotype := levels(fig2c_r$genotype)]

# create a pretty p-value column
aov2_pairs_dt <- data.table(aov2_pairs)
aov2_pairs_dt[, pretty_p := lazyWeave::pvalString(p.value)]

# add group1 and group2 columns to aov2_pairs_dt
# again, aov_4 didn't like group names so replace contrasts
# with those from the lmm1 fit
aov2_pairs_dt[, contrast := lmm1_pairs$contrast]
# now copy the first part of each contrast into group 1 and 2nd part into group 2
aov2_pairs_dt[, group1 := c("PD1+TIM3-", "PD1-TIM3-", "PD1-TIM3-")]
aov2_pairs_dt[, group2 := c("PD1+TIM3+", "PD1+TIM3+", "PD1+TIM3-")]
...
gg1 <- ggplot(data = fig2c_r,
  ...
  geom_line(aes(group = experiment_id),
            position = position_dodge(width = 0.2),
            color = "gray40") +
  geom_point(data = aov2_emm_dt,
             aes(y = emmean,
                 color = genotype),
             size = 3) +
  geom_errorbar(data = aov2_emm_dt,
                aes(y = emmean,
                    ymin = lower.CL,
                    ymax = upper.CL,
                    color = genotype),
  ...
  #pvalue brackets
  stat_pvalue_manual(aov2_pairs_dt,
                     label = "pretty_p",
...
1. If our rmANOVA model is fit with the default specification (`exp5c_aov2`), we can get the multivariate output using the `model = "multivariate"` argument within `emmeans` (not the `contrast` function!). Or, if our fit excluded the univariate model (`exp5c_aov1`), then we don't need the `model` argument.

**contrasts from univariate model**

```{r}
# get the univariate model results using $aov
emmeans(exp5c_aov2$aov, specs = c("treatment", "activity")) |>
  contrast(method = "revpairwise",
           simple = "each",
           combine = TRUE,
           adjust = "none")
...
1. Passing `exp5c_aov2` to `emmeans` will return the contrasts from the multivariate. change this to the univariate model by passing `exp5c_aov2$aov`.

#### The ANOVA table

```{r}
nice(exp5c_aov2, correction = "GG")
nice(exp5c_aov2, correction = "none")
```
