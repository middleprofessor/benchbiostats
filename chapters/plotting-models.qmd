...
The response variable in Figure 2e is the "area under the curve" (AUC)--the area under the line connecting the glucose values for an individual mouse at each of the time points (@fig-plots-fig2e-longitudinal). Dividing the AUC by 120 minutes would give the average blood glucose level over the duration of the test.
...
fig2e[, treatment_orig := treatment]
fig2e[, treatment := paste(diet, genotype) |>
        factor(levels = c("Chow WT", "Chow adipoKO", "HFD WT","HFD adipoKO"))]

# for plot only (not analysis!)
shift <- 2
fig2e[treatment_orig == "ASK1F/F chow", time_x := time - shift*1.5]
fig2e[treatment_orig == "ASK1Δadipo chow", time_x := time - shift*.5]
fig2e[treatment_orig == "ASK1F/F HFD", time_x := time + shift*.5]
fig2e[treatment_orig == "ASK1Δadipo HFD", time_x := time + shift*1.5]
...
```{r}
#| label: fig-plots-fig2e-longitudinal
#| echo: false
#| fig-cap: "Blood glucose levels over time course of a glucose tolerance test. Values for each treatment are separated at each time-of-measurement to more easily see group differences."

ggplot(data = fig2e,
              aes(x = time_x, y = glucose, color = treatment)) +
  geom_point() +
...
### Pretty good plot component 1: Response plot
...
fig2e_gg_response_p <- fig2e_gg_response +
...
The response plot in @fig-plots-fig2-response also (shows the data)[https://journals.physiology.org/doi/full/10.1152/advan.00009.2011]{target="_blank"} by plotting response values as "jittered" dots. Showing the data
...
Finally, the response plot in @fig-plots-fig2-response reports the interaction *p*-value, which is almost always the focal *p*-value that we want in a 2 x 2 factorial experiment like this, but is almost never reported.


```{r}
#| label: tbl-plots-model-means-table
#| echo: false
#| tbl-cap: Sample and model means and standard errors for the Figure 2e data.
...
### Pretty good plot component 2: Effects plot
...
```{r}
#| label: fig-plots-fig2e-ggplot
#| echo: false
#| fig-cap: "Effect of diet and ASK1 deletion on post-baseline glucose. Top: effects plot of four simple effects (difference in means) and of the diet:genotype interaction. Bars are 95% confidence intervals of the effects. Unadjusted p-values from the linear model are given. Bottom: response plot of the means and 95% confidence interval of each diet:genotype combination."
