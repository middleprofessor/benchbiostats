...
p_methods <- c("None", "Tukey", "Bonferroni", "Sidak",
               "Holm", "MvT", "FDR")
...
Table @tbl-twoway-exp2j-p-adjust shows *p*-values adjusted using seven different methods (including no adjustment) for the Experiment 2j glucose uptake data.

The Tukey correction is applied to all six pairwise tests in the code above.

1. **Tukey** -- The Tukey HSD method adjusts for the family including all pairwise comparisons, which is six tests in a 2 x 2 design. This is the most common adjustment for 2 x 2 designs in the bench biology literature. This is also the default adjustment in the emmeans package. **Importantly**, even if using best-practice mindless statistics and reporting only the simple effects, the researcher will lose some power compared to alternative adjustment methods because the family size is six and not four. 
2. **Bonferroni** -- Bonferroni is less common than Tukey in the experimental bench biology literature but is never recommended because the Holm modification (see below) has higher power without inflated Type I error. The Bonferroni adjustment assumes independence among the tests. It's type I error rate is generally too conservative for 2 x 2 designs.
3. **Sidak** -- The Sidak correction has increased power relative to Bonferroni but does not explicitly account for correlated tests. It's type I error rate is generally too conservative for 2 x 2 designs.
4. **Holm** -- The Holm variant of the Bonferroni adjustment has increased power relative to the Bonferroni but does not explicitly account for correlated tests. It's type I error rate is generally too conservative for 2 x 2 designs.
5. **Mvt** -- The multivariate *t* adjustment corrects for the empirically measured correlation.
6. **fdr** -- The FDR adjustment does not attempt to correct for Type I error but for the False Discovery Rate. The difference between these concepts is important. See @sec-multiple-tests.
