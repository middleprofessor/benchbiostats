...
::: {.callout-warning}
If you have a numeric or integer `x` variable but want the model to treat this as categorical, you need to change the object type to factor, otherwise the lm function will treat the variable as numeric. For example, if you have a variable `time` with levels 1, 2, 3 instead of "1 hour", "2 hours", "three hours", then you could fit this as a factor variable using the formula `y ~ factor(time)` to force R to create the indicator variables. I prefer to explicitly create a version of the variable that is recoded as factor using something like `dt[, time_fac := factor(time, levels = c("1", "2", "3"))]`
:::
...
In any experiment with more than two groups, we have multiple contrasts (or "multiple tests"). Almost always, researchers are only interested in a subset of these. We can limit the number of contrasts computed by the `contrast()` function by passing a custom contrast object in `method =`.
