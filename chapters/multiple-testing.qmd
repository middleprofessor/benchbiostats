...
The base R `p.adjust()` function adjusts a set of *p*-values using any of several methods.

Let's use the lipocalin Fig3 data. First get the p values for the four simple effects on the cardiolipin in panel d. Be sure to use the `adjust = "none` argument because we want to adjust unadjusted *p*-values!
...
1. "BH" or "fdr" is a method for controlling the FDR using the logic of Benjamini & Hochberg. The BH method assumes independent tests. This or BY should be used for most uses of *p*-value adjustment in experimental bench biology.
...
3. "holm" is the Holm modification of Bonferroni and is used to control the FWER. It is more powerful than Bonferroni while still holding FWER at alpha. For most uses of *p*-value adjustment in experimental bench biology, we are less interested in controling the FWER than the FDR.
4. "bonferroni" is the original, general-purpose adjustment for a set of *p*-values to control the FWER. It has less power than the Holm modification, so it's use should be historic only.

### Adjusting p values in the emmeans contrast() function

For individual responses, we don't need to use the p.adjust() function because it is built into the contrast() function that we use for inference.
