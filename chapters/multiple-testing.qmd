...
For individual responses, we don't typicall use the `p.adjust()` function because it is built into the `contrast()` function that we use for inference. The `contrast()` functional has additional adjustment methods useful for experimental designs.
...
4. "mvt" -- based on the multivariate *t* distribution and using covariance structure of the variables.
