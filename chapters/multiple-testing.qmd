...
## Example 3 -- When we do want to adjust, we often want to use a method that controls the False Discovery Rate (FDR) (lipocalin fig3 data)
...
The researchers are investigating the role of *Lipocalin-2* (*LCN2*) on mitochondrial membrane function. Using a LCN2 knockout and the thermogenic molecule CL316,243, the researchers scanned for differences in levels of 246 lipids between WT and KO mice with and without CL316,243 treatment. Of the different classes of lipids, the biggest differences were found in cardiolipins. The panels in Fig 3d-u show cardiolipin levels among treatment groups for 18 of the 26 cardiolipins with at least one significant *p*-value for a difference between WT and KO mice. The researchers organize the panels into distinct patterns of response to treatment. This suggests the research question is something like, "do we see differeces in CL synthesis under different combinations of LCN2 activity and CL316,243?". There are 4 x 26 contrasts that answer this -- the four simple effects within a cardiolipin response across all 26 cardiolipins tested.
...
if(pool_contrast == TRUE){
  ...
  # us lm pvalue instead of t test pvalue for increased power
...
data.table(
  "Method" = c("lm", "ttest", "holm", "fdr_BH", "fdr_BY"),
...
The adjumstment method for both the confidence interval and the *p*-value is given in the contrast object.
