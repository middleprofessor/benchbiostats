...
The probability of a significant *p*-value in one test is 0.05, so the probability of no non-significant *p*-values in one test is 1 - 0.05. The probability of no non-significant *p*-values in two tests is $(1 - 0.05)^2$, so the probability of at least one significant *p*-value is $1 - (1 - 0.05)^2$. The probability of no non-significant *p*-values in ten tests is $(1 - 0.05)^10$, so the probability of at least one significant *p*-value in ten tests is $1 - (1 - 0.05)^2$. Let's compute these probabilities, and more. 
...
## In many experiments, we don't want to adjust for multiple tests
...
Let's look at the cost of adjustment. The researchers used the Holm-Sidák method to adjust the two *p*-values. Holm-Sidák is an option in Graphpad Prism but not Base R and is not available in any of the major testing R packages. So, I wrote a function to compute this.

```{r}
#| label: mult-test-holm-sidak
#| echo: true
...
## Tukey adjustment is extremely common, and (mostly) unjustified
...
fig5l[treatment_orig == "flox-veh", treatment := "WT Vehicle"]
fig5l[treatment_orig == "flox-Pue", treatment := "WT Puerarin"]
fig5l[treatment_orig == "KO-veh", treatment := "KO Vehicle"]
fig5l[treatment_orig == "KO-Pue", treatment := "KO Puerarin"]
fig5l[, treatment := factor(treatment, levels = unique(treatment))]

```


A very common research design in experimental bench biology is a design with two factors, each with two levels, with mice assigned to all 2 x 2 = 4 combinations of the levels. Analysis of these models for two (or more) factors is the focus of Chapter xxx. The experiment in Figure 5l is a good example of a 2 x 2 design.
...
Of the six pairwise contrasts, the authors are focussed on four, the so-called "simple effects".
...
#| 
m1 <- lm(ezrin ~ treatment, data = fig5l)
m1_emm <- emmeans(m1, specs = "treatment")
m1_pairs <- contrast(m1_emm,
                     method = "revpairwise",
                     adjust = "none") |>
...
m1_pairs[ , multiple := tukey/p.value]

m1_pvals <- m1_pairs[, .SD, .SDcols = c("contrast", "estimate", "p.value", "tukey", "multiple")]
colnames(m1_pvals) <- c("Contrast", "Estimate", "Unadjusted P", "Tukey P", "Tukey/Unadjusted")

# only the simple effects are of interest. These are rows 1, 2, 5, 6
m1_pvals[c(1, 2, 5, 6), ] |>
  kable(digits = c(1, 3, 4, 4, 1)) |>
  kable_styling(full_width = FALSE)

```

The cost of adjustment is given in the last column of @tbl-mult-tests-fig5_m1. In three of the contrasts, the Tukey adjusted *p*-value is 5-6X the unadjusted value. This is an unnecessary loss of power. Loss of power leads to more time, more dollars, more animals, and more failed discoveries.  
...
## Using the FDR method to adjust *p*-values from different fit models, each for a different response
...
if(write_it == TRUE){
  ...
  for(fig_id in letters[4:21]){
    ...
    if(fig_id == letters[4]){
      ...
      fig3 <- merge(fig3, fig3_part, by = c("mouse_id", "treatment"))
    }
    # fig3 <- rbind(
    #   fig3,
    #   data.table(
...
fig3 <- read_excel(outfile_path) |>
  data.table()
fig3[, treatment := factor(treatment, levels = unique(treatment))]

```


```{r}
#| label: multiple-tests-fig3-analyze
#| echo: false
#| message: false
#| warning: false
#| eval: true
...
n_cardiolipins <- length(y_cols)

p_table <- data.table(NULL)
for(set_i in 1:n_cardiolipins){
  lipid_id <- colnames(fig3)[y_cols[set_i]]
  formula_i <- paste(lipid_id, "~", "treatment") |>
    formula()
  ...
  p1 <- t.test(fig3[treatment == "WT-Saline", get(lipid_id)],
               fig3[treatment == "WT-CL", get(lipid_id)],
               var.equal = TRUE)$p.value
  p2 <- t.test(fig3[treatment == "WT-Saline", get(lipid_id)],
               fig3[treatment == "KO-Saline", get(lipid_id)],
               var.equal = TRUE)$p.value
  p3 <- t.test(fig3[treatment == "KO-Saline", get(lipid_id)],
               fig3[treatment == "KO-CL", get(lipid_id)],
               var.equal = TRUE)$p.value
  p_table <- rbind(
    p_table,
    data.table(
