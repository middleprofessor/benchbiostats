# Multiple tests -- why and when to adjust *p*-values {#sec-multiple-tests}
...
## In a family of multiple tests, what is the probability of at least one p-value less than 0.05? {#sec-fwer}
...
The import code is in @sec-exfig2-import-unhide below.
...
## Example 2 -- Tukey adjustment is extremely common, and (mostly) unjustified (fig5l data) {#sec-mult-tests-tukey}
...
The import code is in @sec-fig5l-import-unhide below
...
## Example 3 -- When we do want to adjust, we often want to use a method that controls the False Discovery Rate (FDR) (lipocalin fig3 data) {#sec-mult-tests-eg3-fdr}

::: {.callout-note title="Wait What?"}
...
Remember that the family wise error rate (FWER) is the long term probability of at least one $p < 0.05$ in a batch of tests in which all Null Hypothesis are true -- there are no true effects (@sec-fwer).
...
The import code is in @sec-fig3-import-unhide below
...
::: {.callout-note title="Which adjustment method?"}
...
```{r}
#| label: tbl-multiple-tests-fig3-pvals
#| echo: true
#| eval: true
#| message: false
#| warning: false
#| tbl-cap: Number of discoveries (*p* < 0.05) among the 104 tests
...
Notes on @tbl-multiple-tests-fig3-pvals
...
## Better Know FDR {sec-mult-tests-fdr}
...
Notes on @tbl-mult-tests-fdr-simulation
...
## Working in R
### The contrast() function has different default adjustment methods for each of the contrast methods

1. Tukey HSD is the default for all pairwise contrasts using either `method = "pairwise"` or `method = "revpairwise"`

```{r}
#| label: multiple-tests-working-revpairwise-tukey
...
```{r}
#| label: multiple-tests-working-revpairwise-dunnett
...
### For non-default adjustment, use the `adjust = ` argument

For no adjustment, use `adjust = "none"`

```{r}
#| label: multiple-tests-working-revpairwise-none
...
### Options in `adjust = ` {#sec-mult-tests-adjust-options}
...
### A Holm-Sidak function for reproducing GraphPad Prism results
...
```{r}
#| label: multiple-tests-holm-sidak-check
#| eval: false
#| echo: false
...
# compute holm-sidak
m1_simple[, holm_sidak := holm_sidak(p.value)]
p_check <- RHSDB::rh.sd.sidak(m1_simple$p.value,
                              effect = m1_simple$estimate,
                              effect.se = m1_simple$SE,
                              df = m1_simple$df)
m1_simple <- cbind(
  m1_simple, p_check
)
m1_simple |>
  kable(digits = c(1,1,1,3,3,1,3,3,1,4,4,4,4,4,4)) |>
  kable_styling()
```


## Hidden Import Code
### Example 1 (exfig2 data) {#sec-exfig2-import-unhide}

```{r}
#| label: multiple-tests-exfig2-import-unhide
#| echo: true
#| message: false
#| warning: false
...
### Example 2 (fig5l data) {#sec-fig5l-import-unhide}

```{r}
#| label: multiple-tests-fig5l-import-unhide
#| echo: true
#| message: false
#| warning: false
data_from <- "A brain-to-gut signal controls intestinal fat absorption"
...
### Example 3 (fig3 data) {#sec-fig3-import-unhide}

```{r}
#| label: multiple-tests-fig3-import-unhide
#| echo: true
#| message: false
#| warning: false
#| eval: false
