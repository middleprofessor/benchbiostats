# Linear models for counts, binary responses, skewed responses, and proportions -- Generalized Linear Models {#sec-glm}
...
Biologists frequently count stuff, and design experiments to estimate the effects of different factors on these counts. Count data can cause numerous problems with linear models that assume a normal, conditional distribution, including 1) counts are discrete, and can be zero or positive integers only, 2) counts tend to bunch up on the small side of the range and spread thinly on the large side of the range, creating a distribution with a positive skew, 3) a sample of counts can have an abundance of zeros, and 4) the variance of counts increases with the mean (see @fig-glm-fig3a-histogram for some of these properties). Some count data can be approximated by and reasonably modeled with a normal distribution. More often, count data should modeled with a [Quasipoisson distribution](https://esajournals.onlinelibrary.wiley.com/doi/abs/10.1890/07-0043.1){target="_blank"} or [Negative Binomial distribution](https://en.wikipedia.org/wiki/Negative_binomial_distribution){target="_blank"} using a [generalized linear model](https://en.wikipedia.org/wiki/Generalized_linear_model (GLM)){target="_blank"}. Quasipoisson and Negative Binomial distributions are **discrete probability distributions** with two important properties: 1) the distribution contains only zeros and positive integers and 2) the variance is a function of the mean. Back before modern computing and fast processors, count data were often analyzed by either **transforming** the response or by **non-parametric hypothesis tests**. One reason to prefer a statistical modeling approach with a GLM is that we can get interpretable parameter estimates better behaved error intervals. 
...
#| fig-width: 4
#| fig-height: 3
#| fig-cap: "Histogram of the number of angiogenic sprouts in response to two of the four treatment combinations for the experiment in Example 1."

gghistogram(data = fig3a[genotype == "FAK_wt"],
            x = "sprouts",
            color="t_by_g",
            fill="t_by_g",
            add = "mean",
...
That is, we should not think of a count as the sum of a systematic part and a random draw from a Poisson distribution. Why? Because it is the counts, conditional on the $\texttt{treatmentgenotype}$ combination that are quasipoisson distributed, not the residuals from the fit model. A residual is a difference from a mean, so, some residuals from a fit model **have to be negative**. A negative residual cannot be a sample from a quasipoisson or Negative Binomial distribution because these can only generate non-negative integers.
...
## Example 1 -- GLM models for count responses ("angiogenic sprouts" fig3a) {#glm-1}
### Understand the data
...
fig3a_qp <- glm(sprouts ~ treatment * genotype,
                ...
                data = fig3a)
fig3a_nb <- glm.nb(sprouts ~ treatment * genotype,
                data = fig3a)
```

Notes on the models

1. The Poisson almost never (ever?) fits bench biological data well. I'm using it here to show this with the model check.
2. The Quasipoisson and Negative Binomial distributions tend to fit biological data well and are similare to each other. I find that QP improves test performance in simulations, so prefer it for statistical inference. Unfortunately, the Quasipoisson is not implemented in the model check functions.
...
```{r glm-fig3a_pois-check-poisson}
# from the DHARMa package
  fig3a_pois_check <- simulateResiduals(fittedModel = fig3a_pois, n = 250)
  plot(fig3a_pois_check, asFactor = FALSE)
```
...
```{r glm-fig3a_pois-check-dispersion}
fig3a_pois_check_refit <- simulateResiduals(fittedModel = fig3a_pois,
                                       n = 250,
                                       refit = TRUE)
fig3a_pois_test_dispersion <- testDispersion(fig3a_pois_check_refit)
...
```{r glm-fig3a_pois-check-zero}
fig3a_pois_test_zi <- testZeroInflation(fig3a_pois_check_refit)
```
...
```{r glm-fig3a_nb-check-nb}
# from the DHARMa package
  fig3a_nb_check <- simulateResiduals(fittedModel = fig3a_nb,
                                        n = 250)
  plot(fig3a_nb_check, asFactor = FALSE)
...
```{r glm-fig3a_nb_test_dispersion}
fig3a_nb_check_refit <- simulateResiduals(fittedModel = fig3a_nb,
                                               n = 250,
                                               refit = TRUE)
fig3a_nb_test_dispersion <- testDispersion(fig3a_nb_check_refit)
...
```{r glm-fig3a_nb_test_zi}
fig3a_nb_test_zi <- testZeroInflation(fig3a_nb_check_refit)
```

#### Inference from the model {#glm-fig3a-m3-inference}

```{r glm-fig3a_nb-coef, message=FALSE}
fig3a_nb_coef <- cbind(coef(summary(fig3a_nb)),
                       confint(fig3a_nb))

```

```{r glm-fig3a_nb-coef-show, echo=FALSE}
fig3a_nb_coef %>%
...
```{r glm-fig3a_nb_emm}
fig3a_nb_emm <- emmeans(fig3a_nb,
                        specs = c("treatment", "genotype"),
                        type="response")
```


```{r glm-fig3a_nb_emm-show, echo=FALSE}
fig3a_nb_emm %>%
  kable(digits = c(1,1,2,1,1,2,2)) %>%
  kable_styling()
```

```{r glm-fig3a_nb_planned}
# fig3a_nb_emm # print in console to get row numbers
...
```{r glm-fig3a_nb_planned-show, echo=FALSE}
fig3a_nb_planned %>%
...
ggplot_the_response(fig3a_nb,
                 fig3a_nb_emm,
                 fig3a_nb_planned[1:2,],
                 y_label = "Sprouts",
                 contrast_rows = c(1:2),
                 palette = pal_okabe_ito_blue)
...
```{r glm1-fake-poisson, fig.width=std_width, fig.asp=0.5, echo=FALSE, warning=FALSE}
n_samp <- 10^4
...
```{r glm1-fake-nb, fig.width=std_width, fig.asp=0.5, echo=FALSE, warning = FALSE, caption = "Samples from Negative Binomial with different values of mu and k parameters. Note the different scales of the x and y axes."}
n_samp <- 10^4
...
3. Quasipoisson -- The Quasipoisson distribution is similar to the NB distribution in that it models overdispersion, which occurs when the variance increases faster than the mean. In the NB, the overdispersion parameter $\theta$ is a function of the mean which makes the variance increase with the square of the mean. In the QP, the overdispersion parameter $\theta$ is not a function of the mean, so the variance increases linearly with the mean.
...
Normal-QQ plots were introduced in Section \@ref{normal-qq} of the Model Checking chapter and applied to the linear model fit of the angiogenic sprout data in Section \@ref(glm-fig3a-normal-qq) above. We cannot use a Normal-QQ plot with a Poisson or Negative Binomial GLM fit because the residuals from this fit are not expected to be normally distributed. An alternative to a Normal-QQ plot for a GLM fit is a **quantile-residual uniform-QQ** plot of observed **quantile residuals**.

```{r glm-fig3a_pois-check-poisson-again, fig.dim=small_dim, echo=FALSE, fig.cap = "Quantile-residual uniform-QQ plot of the Poisson GLM fit to the angiogenic sprouts (fig3a) data."}
simulation_output <- simulateResiduals(fittedModel = fig3a_pois,
                                       n = 250,
                                       refit = FALSE)
plotQQunif(simulation_output)

# ggcheck_the_glm_qq(fig3a_pois, se = TRUE)
```
...
```{r glm-fig3a_pois-spreadlevel-again, fig.dim=small_dim, echo=FALSE, fig.cap = "Spread-level plot of the Poisson GLM fit to the angiogenic sprouts (fig3a) data.", echo=FALSE}
plotResiduals(fig3a_pois, asFactor = FALSE, quantreg = TRUE)
```
...
2. Why fit the three quartiles? The classic spread-level plot typically fits a regression, which is the expected (mean) residual, conditional on the fitted value (for example, the plot in @fig-glm-fig3a-sl). Researchers typically look at this and think, "the whole spread of points is becoming more variable as the fitted value increases" but the regression is only modeling the mean. By fitting the first, second, and third quartile regressions, this spread-level plot evaluates deviations from expected spread not just at the mean but in the lower and upper halves of the spread. 

#### Model checking a GLM III -- Checking dispersion

```{r glm-fig3a_pois-check-dispersion-again, fig.dim=small_dim, echo=FALSE, fig.cap = "Dispersion plot of the Negative Binomial GLM fit to the angiogenic sprouts (fig3a) data.", echo=FALSE}
simulation_output <- simulateResiduals(fittedModel = fig3a_pois,
                                       n = 250,
                                       refit = TRUE)
...
```{r glm-fig3a_nb-qq-again, fig.dim=small_dim, echo=FALSE, fig.cap = "Quantile-residual uniform-QQ plot of the Negative Binomial GLM fit to the angiogenic sprouts (fig3a) data.", echo=FALSE}
simulation_output <- simulateResiduals(fittedModel = fig3a_nb,
                                       n = 250,
                                       refit = FALSE)
plotQQunif(simulation_output)
...
```{r glm-fig3a_nb-spreadlevel-again, fig.dim=small_dim, echo=FALSE, fig.cap = "Spread-level plot of the Negative Binomial GLM fit to the angiogenic sprouts (fig3a) data.", echo=FALSE}
plotResiduals(fig3a_nb, asFactor = FALSE, quantreg = TRUE)

```

```{r glm-fig3a_nb-check-dispersion-again, fig.dim=small_dim, echo=FALSE, fig.cap = "Dispersion plot of the Negative Binomial GLM fit to the angiogenic sprouts (fig3a) data.", echo=FALSE}
simulation_output <- simulateResiduals(fittedModel = fig3a_nb,
                                       n = 250,
                                       refit = TRUE)
...
1. In the linear model `fig3a_lm`, the intercept is the modeled mean of the reference group. In a GLM with a log-link (including Models `fig3a_pois` and `fig3a_nb`), the intercept is the mean of the reference group *on the link scale*. The modeled mean of the reference on the response scale is computed by the backtransformaiton $\exp(b_1)$. The function "exp(x)" is the exponent, which is often written using the notation $e^{x}$.
2. The transformation between link scale and response scale is part of the specification of a Generalized Linear Model. For the Negative Binomial GLM fit to the fig3a data, this specification is in line 3 of the specification (Model \@ref(eq:nb-glm-fig3a)).

Compare the computation here with the modeled mean in the emmeans table (Section \@ref(glm-fig3a-m3-inference) or with a computation of the sample mean.

```{r glm1-fig3a_nb-b1}
b1 <- coef(fig3a_nb)[1]
exp(b1)
```
```{r glm1-fig3a_nb-mean}
...
```{r glm1-fig3a_nb-emm_link}
fig3a_nb_emm_link <- emmeans(fig3a_nb,
                             specs = c("treatment", "genotype")) %>%
  summary() %>%
  data.table()
```
...
```{r glm1-fig3a_nb-diff}
fig3a_nb_emm_link[treatment == "GAS6" &
                    genotype == "FAK_wt", emmean] -
  fig3a_nb_emm_link[treatment == "PBS" & genotype == "FAK_wt", emmean]
...
```{r glm1-fig3a_nb-emm_response}
fig3a_nb_emm_response <- emmeans(fig3a_nb,
                             specs = c("treatment", "genotype"),
                             type = "response") %>%
  summary() %>%
  data.table()
...
```{r glm1-fig3a_nb-ratios}
fig3a_nb_emm_response[treatment == "GAS6" &
                    genotype == "FAK_wt", response]/
  fig3a_nb_emm_response[treatment == "PBS" & genotype == "FAK_wt", response]
  
```

And the coefficient for the $\texttt{treatment_gas6}$ indicator variable backtransformed to the response scale is

```{r glm1-fig3a_nb-exp_b2}
exp_b2 <- exp(b2)
exp_b2
```
...
```{r fig3a_nb_emm_link, echo=FALSE}
fig3a_nb_emm_link %>%
...
```{r glm1-fig3a_nb-means_link}
b <- coef(fig3a_nb) # the model coefficients
...
Compare the values here to those in the emmeans table on the link scale (Table \@ref(tab:fig3a_nb_emm_link)).

#### The emmeans table on the response scale contains more readily interpretable means

```{r fig3a_nb_emm_response, echo=FALSE}
fig3a_nb_emm_response %>%
  kable(digits = c(1,1,5,2,1,2,2),
...
Compare the values here to those in the emmeans table on the response scale (Table \@ref(tab:fig3a_nb_emm_response)).
...
fake_fig3a[, c("treatment", "genotype"):= tstrsplit(t_by_g,
                                             " ",
                                             fixed = TRUE)]
...
mu_vec <- rep(fig3a_nb_emm_response[, response], each = 100)
theta_sim <- fig3a_nb$theta
fake_fig3a[, sprouts_nb := rnegbin(n = n*4,
                                mu = mu_vec,
                                theta = theta_sim)]
...
## Example 2 -- Use a GLM with an offset instead of a ratio of some measurement per total ("dna damage" data fig3b) #glm_offset
...
#### Inference from the model {#glm-fig3b-m3-inference}
...
2. Figure \@ref(fig:glm-eg2-fig3b-m3-plot-offset) illustrates the offset on the response and the log scale.
...
```{r glm-eg2-fig3b-m3-plot-offset, echo=FALSE, fig.cap = "What an offset looks like. A. The offset on the response scale. B. The offset on the log scale. In A and B, only the lines for the 20 month old mice are shown."}
plot_grid(gg1, gg2, ncol = 2, labels = "AUTO")
```

3. On the log scale (Figure \@ref(fig:glm-eg2-fig3b-m3-plot-offset)B), the offset curves are parallel because each is forced to have a slope of one on the link scale. On the response scale, these curves have different slopes -- this slope is $\mathrm{exp}(b_0)$, that is, the exponent of the intercept on the log scale).
4. The plot on the log scale (Figure \@ref(fig:glm-eg2-fig3b-m3-plot-offset)B) is easy to misinterpret as a regression with a slope of 1 fit to the log transformed values. It isn't, because individual values are not transformed in a GLM. I'm specifically referring to this space as the log and not link scale to try to avoid this misinterpretation.
...
1. The coefficient table for Model fig3b_m5 has a row for the added covariate $\texttt{area_mm2}$. This row is absent for Model fig3b_m3 since the coefficient is fixed at 1.0.
2. The estimate of the coefficient of $\texttt{area_mm2}$ in Model fig3b_m5 is 2.48. A value of 1.0 -- the value assumed by the offset model or an analysis of the ratio -- is not very compatible with the data.
...
4. This increase in the rate of TUNEL positive cells is seen in the response-scale plot of the fit model in Figure \@ref(fig:glm-fig3b-m5-plot)A, where the slope of the regression line increases as $\texttt{area_mm2}$ increases.
