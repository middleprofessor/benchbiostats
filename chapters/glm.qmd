...
Biologists frequently count stuff, and design experiments to estimate the effects of different factors on these counts. Count data can cause numerous problems with linear models that assume a normal, conditional distribution, including 1) counts are discrete, and can be zero or positive integers only, 2) counts tend to bunch up on the small side of the range and spread thinly on the large side of the range, creating a distribution with a positive skew, 3) the variance of counts increases with the mean (see @fig-glm-fig3a-histogram for some of these properties).
...
Generalized linear mixed models (GLMMs) have frequent convergence issues. If this happen, use a linear mixed model of the untransformed data instead of a GLM of a reduced model as the pseudoreplication of a reduced model is (generally) a massively larger problem than data that aren't approximately normal.
:::
