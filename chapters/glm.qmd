...
3. If a response is a fraction that is a ratio of counts, for example the fraction of total cells that express some marker, then use the binomial family with a logistic link. Think of each numerator count as a "success".
4. If a response is a fraction of counts per "effort" (cells per area or volume or time), then use a Poisson, quasi-Poisson or Negative Binomial family with a log link on the raw count and include the log of the measure of effort as an offset.
...
where $\mu$ is the mean response ($\texttt{positive_nuclei}$) conditional on the level of genotype. Statisticians typically refer to the ratio $\frac{\mu}{\texttt{area\_mm2}}$ as a rate -- here, it is the expected rate of observing a TUNEL+ nucleus as more area is searched.
...
Here, I use `ggcheck_the_glm` which uses DHARMa underneath the hood. DHARMa does not simulate Quasipoisson model fits, but I have included these in `ggcheck_the_glm`. Because there is no native Quasipoisson check, I refit the fig3a data using a negative binomial

```{r}
fig3a_qp_1 <- glmmTMB(sprouts ~ treatment * genotype,
                family = nbinom1(link = "log"),
                data = fig3a)
fig3a_qp_2 <- glm(sprouts ~ treatment * genotype,
                family = quasipoisson(link = "log"),
                data = fig3a)
fig3a_nb_1 <- glmmTMB(sprouts ~ treatment * genotype,
                family = nbinom2(link = "log"),
                data = fig3a)
fig3a_nb_2 <- glm.nb(sprouts ~ treatment * genotype,
                data = fig3a)
coef(summary(fig3a_qp_1))
coef(summary(fig3a_qp_2))

contrast(emmeans(fig3a_qp_1, specs = c("treatment", "genotype"),
                 type = "response"),
         method = "revpairwise", simple = "each", combine = TRUE, adjust = "none")
contrast(emmeans(fig3a_qp_2, specs = c("treatment", "genotype"),
                 type = "response"),
         method = "revpairwise", simple = "each", combine = TRUE, adjust = "none")
contrast(emmeans(fig3a_nb_1, specs = c("treatment", "genotype"),
                 type = "response"),
         method = "revpairwise", simple = "each", combine = TRUE, adjust = "none")
contrast(emmeans(fig3a_nb_2, specs = c("treatment", "genotype"),
                 type = "response"),
         method = "revpairwise", simple = "each", combine = TRUE, adjust = "none")
...
```{r}
fig3a_qp_check <- ggcheck_the_glm(fig3a_qp, n_sim = 250) # 250 fake data sets
