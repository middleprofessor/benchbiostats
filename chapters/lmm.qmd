...
if(error_type == "type1"){
  ...
  if(correlated_model & temp == FALSE){
    outfile_name <- "rcbd_type1-cor-model.Rds"
  }
  if(correlated_model == FALSE & temp == FALSE){
    outfile_name <- "rcbd_type1-cor-model.Rds"
  }
  # parameters
  beta_i = c(10, 0, 0)
...
if(error_type == "power"){
  ...
  if(correlated_model & temp == FALSE){
    outfile_name <- "rcbd_power-cor-model.Rds"
  }
  if(correlated_model == FALSE & temp == FALSE){
    outfile_name <- "rcbd_power-cor-model.Rds"
  }
  ...
  for(param_set in 1:nrow(param_set_matrix)){
    ...
    for(i in 1:n_sims){
      ...
      aov2_pairs <- emmeans(aov2, specs = "treatment", model = "univariate") |>
        contrast(method = "revpairwise", adjust = "none")
      pptt_pairs <- pptt(form_lmm1, data = fake_rcbd_means)
...
3. When there is heterogenous correlated error, only aov1/pptt/lmm2 have reasonable type I error control (again, lmm2 is very slightly anticonservative). lmm1/aov1 have liberal Type I control in contrasts with low correlated error and conservative Type I control in contrasts with high correlated error.
...
1. When there is zero correlated error, lmm2 has slightly higher power than lmm1/aov2/lm1. And lmm1/aov2/lm1 have slightly higher power than aov1/pptt.
2. When there is equal correlated error, lmm2 and lmm1/aov2/lm1 have slightly higher power than aov1/pptt.
...
### aov1 -- repeated measures ANOVA (RM-ANOVA)
...
3. aov1 fits an ANOVA model even though there are only two groups. Yes ANOVA can be used with only two groups, and the statistics are exactly the same as with a *t*-test, but most people choose *t*-test and not ANOVA when there are only two groups.
...
aov1 <- aov_4(pd1_mfi ~ genotype + (genotype | experiment_id), data = fig3e)
aov1_emm <- emmeans(aov1, specs = "genotype")
aov1_pairs <- contrast(aov1_emm, method = "revpairwise") |>
  summary(infer = TRUE)
...
## Example 3: RCBD with > 2 groups (classical equivalent: repeated measures ANOVA) (fig6g) {#sec-lmm-example2}
...
### aov1 -- repeated measures ANOVA (RM-ANOVA), multivariate model

```{r}
aov1 <- aov_4(nr4a1 ~ genotype + (genotype | experiment_id), data = fig2c_r)
aov1_emm <- emmeans(aov1, specs = "genotype")
...
### aov2 -- repeated measures ANOVA (RM-ANOVA), univariate model
...
1. aov1 and pptt1 are equivalent. This is generally true if there are no missing data (genotype:experiment_id combinations). Both aov1 and pptt1 require balance but pptt1 is a pairwise procedure while aov1 analyzes all treatment levels simulataneously. Consequently pptt1 will exlude fewer measures and will have slightly more power than aov1. 
2. aov1 and lmm1 are not equivalent and this is generally true. 
3. aov2 and lmm1 are equivalent and this is generally true if there are no missing data.
4. The lmm equivalent of aov1 is lmm3 (random intercept and slopte model) but this model cannot be fit without replication with genotype:experiment_id combinations.
5. One reasonable conclusion of item 4 is that aov1 effectively overfits the data and we should use aov2/lmm1 as the model of choice. The simulation under RCBD above argues against this. Instead, aov1/lmm3 is preferred (although lmm3 cannot be fit without replication)
6. Item 5 raises the question, why bother with linear mixed models in a RCBD?

### Plot the model

```{r}
plot_response(aov1, aov1_emm, aov1_pairs,
              join_blocks = TRUE,
              block_id = "experiment_id")
```
