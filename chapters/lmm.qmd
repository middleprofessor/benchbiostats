...
Given the basic principles above, let's consider the kinds of experimental designs seen in experimental bench biology (@fig-lmm-designs). For each of the experimental designs, I give examples with a certain number of treatments (t), blocks (b), treatment replications (r), and subsampled replications (s).
...
##### linear model of batch means (aggregated linear model)
...
##### random interaction model for two factor designs
...
rmaov.uni <- aov_4(y ~ treatment + (treatment | block),
           include_aov = TRUE,
             data = figx)
# requires the argument model = "univariate" in the emmeans function
```

##### Univariate repeated measures ANOVA for two factor designs
...
```{r, class.source = "fold-show", eval=FALSE}
rmaov.uni2x <- aov_4(y ~ factor1 * factor2 + (factor1 * factor2 | block),
           include_aov = TRUE,
             data = figx)
# requires the argument model = "univariate" in the emmeans function
```
...
##### Repeated measures ANOVA for Split Plot
...
1. lmm.ri -- random intercept model
2. lm.agg -- linear model of batch means 
3. naov -- nested ANOVA

Notes

1. These are numerically equivalent if the design is balanced

### RCBD
#### Statistical models for single factor RCBD

1. lm.fe -- fixed effect model
2. lmm.ri -- random intercept model
3. lmm.ce -- lmm for correlated error
4. rmaov.mul -- multivariate RM-ANOVA
5. rmaov.uni1 -- univariate RM-ANOVA
...
1. If the number of treatments = 2, *all six methods are numerically equivalent*. A paired *t*-test is a special case of a linear mixed model.
2. If the number of treatments > 2, then
...
1. If a treatment within a block is missing, the whole block is deleted in the rmoav models. This reduces power (the loss of power depends partly on the number of missing values).
2. If a treatment within a block is missing, the block is deleted only in the comparisons including the missing treatment in the pairwise, paired t-tests. This makes ppttm more powerful than rmaov.mul
3. No blocks are deleted in linear mixed models.

Some general notes

1. the lmm models are very flexible -- covariates can be added or these can be used as generalized lmms for non-normal distributions.
2. the lmm models sometimes fail to converge, especially with small samples (small number of blocks) or if the among-block component or block:treatment components of variance are small.

#### Additional models for two factor RCBD

7. lmm.rii2 -- random interaction intercept model with each factor:block intercept
8. rmaov.uni2 -- two-factor univariate RM-ANOVA

Equivalent models if the design is balanced

1. lm.fe, lmm.ri, and rmaov.uni1 are equivalent
2. lmm.rii2 and rmaov.uni2 are equivalent
3. rmaov.mul and ppttm are equivalent
4. lmm.ce is equivalent to rmaov.mul/ppttm, depending on how one models the df

#### Which model and why

1. For single factor RCBD with only two treatment levels, all models are the same if the design is balanced so it doesn't matter. If the design is unbalanced, lmm.ri will have more power because no block:treatment combinations are excluded.
2. For single factor RCBD with more than two treatment levels, lmm.ce/rmaov.mul/ppttm control type I error well
3. For two factor RCBD...

### RSPD


1. lmm.sp1 - random intercept model for CRD split plot designs
2. lmm.sp2 - random interaction intercept model of RCBD split plot designs
3. rmaov.sp - repeated measures ANOVA for a split plot design

### RCBDS

1. lmm.ris -- random intercept and slopes model
2. lmm.rii -- random interaction intercept model
3. lmm.ri -- random intercept model on batch means
4. lmm.ce -- lmm for correlated error on batch means
6. rmaov.mul -- multivariate RM-ANOVA
7. rmaov.uni -- univariate RM-ANOVA
8. pptt -- pairwise, paired t-test


Notes

1. lmm.ris is the "maximal model" -- it fits the most parameters and has the fewest assumptions. lmm.ri on the full data is the minimal model. I'm not recommending this at all as this is pseudoreplication and will result in very anti-conservative inference. lmm.rii is less conservative than lmm.ris (and makes more assumptions).
2. lmm.ri and lmm.ce *here* are the models for RCBD designs but using the aggregated data - that is the subsampled replicates within a batch (block:treatment combination) are averaged.
...
6. the lmm models, and especially lmm.ce and lmm.ris, sometimes (often?) fail to converge, especially with small samples (small number of blocks) or if within block correlation is small
7. lmm.ri, lmm.rii, and rmaov.uni will be less conservative (more power but at higher type I error/false positive rate)

Best practices

1. If balanced and no covariates then use lmm.ris/lmm.ce/rmaov.mul if discovery is expensive (want to avoid false positives) or lmm.rii/rmaov.uni/lmm.ri if discovery is cheap (can afford false positives). lmm.ce is less conservative than lmm.ris/rmaov.mul. Use lmm.ris if interested in the variance at different levels and/or covariance structure.
2. If unbalanced, or if there are covariates, then start with lmm.ris or lmm.ce. Use lmeControl if lmm.ce fails. If these fail, go to 1.

### GRCBD

1. lmm.ris -- random intercept and slopes model
2. lmm.rii -- random interaction intercept model
3. lmm.ri -- random intercept model on block:treatment means
4. lmm.ce -- lmm for correlated error on block:treatment means
5. rmaov.mul -- multivariate RM-ANOVA
6. rmaov.uni -- univariate RM-ANOVA
7. lm.fe -- fixed effect model on all data
...
5. rmaov.mul -- multivariate RM-ANOVA
6. rmaov.uni -- univariate RM-ANOVA
7. lm.fe -- fixed effect model on all block:treatment:rep means
8. pptt -- pairwise, paired t-test on block:treatment:rep means

## Example 1: CRD with subsampling (classical equivalent: Nested ANOVA)

Source article: [A new mouse model of Charcot-Marie-Tooth 2J neuropathy replicates human axonopathy and suggest alteration in axo-glia communication](https://journals.plos.org/plosgenetics/article?id=10.1371/journal.pgen.1010477){target="_blank"}
...
```{r lmm-5b-import, message=FALSE, warning=FALSE, echo=FALSE}
data_from <- "A new mouse model of Charcot-Marie-Tooth 2J neuropathy replicates human axonopathy and suggest alteration in axo-glia communication"
file_name <- "journal.pgen.1010477.s014.xlsx"
file_path <- here(data_folder, data_from, file_name)

fig5b_wide <- read_excel(file_path,
                    sheet = "5B",
                    range = "A1:O138") |>
  data.table()
genotype_levels <- c("WT", "TM/+", "TM/TM")
new_cols <- paste(rep(genotype_levels, each = 5), 1:5)
setnames(fig5b_wide, old = names(fig5b_wide), new = new_cols)
fig5b <- melt(fig5b_wide,
              measure_vars = new_cols,
              variable.name = "mouse",
              value.name = "caspr_length")
fig5b[, genotype := tstrsplit(mouse, " ", keep = 1)]
fig5b[, genotype := factor(genotype, levels = genotype_levels)]
fig5b <- na.omit(fig5b)
```


```{r}
#| label: tab-lmm-fig5b-sample-size
#| echo: false
#| tbl-cap: Number of paranodes measured per mouse. Each measurement is a subsampled replicate. Each mouse is a treatment replicate.

fig5b[, .(N = .N), by = .(genotype, mouse)] |>
  kable() |>
  kable_styling()
```
...
1. This model is called a "random intercept" model. The factor $\texttt{mouse}$ is a random factor and is added to the model as a random intercept. Random factors are added inside parentheses. A random intercept is specified with a 1 in front of the bar.
...
1. I would not adjust the *p*-values (see @sec-multiple-tests) but I have adjusted using Tukey HSD to compare to researcher's results in next section
...
### What the researchers did: Classical nested ANOVA
...
1. If the design is balanced (all units have the same sample size), classical nested ANOVA and the linear mixed model with a random intercept are equivalent.
2. In general, ANOVA doesn't handle lack of balance well (different sample sizes per unit). If there were the same number of measures per mouse, The *t* and *p* values of this nested anova would be the same as these values fit by the linear mixed model. As is, the *t* value is slightly smaller and the *p*-value is slightly larger in the nested ANOVA.

### What if the researchers had ignored the non-independence?
...
1. The *p*-values are extremely small because the model "thinks" the data are independent. This is an example of pseudoreplication -- using technical replicates as treatment replicates.

### A deeper dive into the random intercepts
...
## Example 2: RCBD with 2 groups (classical equivalent: paired *t*-test)

source: [Chen, J., López-Moyado, I.F., Seo, H., Lio, C.W.J., Hempleman, L.J., Sekiya, T., Yoshimura, A., Scott-Browne, J.P. and Rao, A., 2019. NR4A transcription factors limit CAR T cell function in solid tumours. Nature, 567(7749), pp.530-534.](https://www.nature.com/articles/s41586-019-0985-x)
...
```{r lmm-fig2c-import, echo=FALSE, message=FALSE}
data_from <- "NR4A transcription factors limit CAR T cell function in solid tumours"
file_name <- "41586_2019_985_MOESM8_ESM.xlsx"
file_path <- here(data_folder, data_from, file_name)

fig2c_import <- function(range_in = "xxx", response_in = ""){
  fig2c_wide <- read_excel(file_path,
                           sheet = "Fig 2",
                           range = range_in,
                           col_names = TRUE) |>
    data.table()
  genotype_levels <- c("TIM3+", "TIM3-")
  colnames(fig2c_wide) <- genotype_levels
  fig2c_wide[, experiment_id := paste("exp_", .I)]
  fig2c_in <- melt(fig2c_wide,
                   id.vars = "experiment_id",
                   variable.name = "genotype",
                   value.name = response_in)
  fig2c_in[, genotype := factor(genotype,
                                levels = genotype_levels)]
  return(fig2c_in)
}
fig2c <- fig2c_import(range_in = "B3:C6", response_in = "nr4a1")
fig2c <- merge(fig2c, 
               fig2c_import(range_in = "B8:C11", response_in = "nr4a2"),
               by = c("genotype", "experiment_id"))
fig2c <- merge(fig2c, 
               fig2c_import(range_in = "B13:C16", response_in = "nr4a3"),
               by = c("genotype", "experiment_id"))
```
...
### rmaov.mul -- repeated measures ANOVA (RM-ANOVA)

```{r lmm-fig2c-rmaov-mul}
...
rmaov.mul_pairs <- contrast(rmaov.mul_emm, method = "revpairwise") |>
  summary(infer = TRUE)
rmaov.mul_pairs
# notice the afex package doesn't like math symbols in group names
```
...
1. All inferential statistics are the same in all three models.
2. A paired *t*-test is a special case of a linear mixed model. A RM-ANOVA with two groups is equivalent to a random intercept linear mixed model.
3. rmaov.mul fits an ANOVA model even though there are only two groups. Yes ANOVA can be used with only two groups, and the statistics are exactly the same as with a *t*-test, but most people choose *t*-test and not ANOVA when there are only two groups.
4. The random component of the model formula for the linear mixed model using `lmer` is `(1 | experiment_id)`. This is the notation for adding a random intercept. $\texttt{experiment\_id$ is a random factor and is added as a random intercept.
5. There is potential for confusion in the formula for the RM-ANOVA model using `aov_4`. The afex package has three equivalent functions for fitting repeated measures ANOVA models, each function is targeted to users with different statistics backgrounds. This text uses the `aov_4` function which uses the `lmer` formula notation. For an RCBD experimental design, `aov_4` uses the `lmer` formula notation for a random intercept AND a random slope, which is `(genotype | experiment_id)`. No random intercept or slope is estimated in an ANOVA model but the notation is necessary to aggregate multiple values of $\texttt{experiment\_id}$ only within and not among $\texttt{genotype}$.
6. For the paired *t*-test, I've used my pptt function which computes paired *t*-tests for all combinations of the factor levels. 

### Plot the model

```{r}
#| label: lmm-fig2c-ggplot
#| echo: false

ggptm(lmm.ri, lmm.ri_emm, lmm.ri_pairs,
      join_blocks = TRUE,
...
```{r}
#| label: lmm-fig2c-ggplot-manual
#| echo: false
#| eval: false

# create a pretty p-value column
lmm.ri_dt <- data.table(lmm.ri_pairs)
lmm.ri_dt[, pretty_p := lazyWeave::pvalString(p.value)]

# now copy the first part of each contrast into group 1 and 2nd part into group 2
lmm.ri_dt[, group1 := c("TIM3-")]
lmm.ri_dt[, group2 := c("TIM3+")]


gg1 <- ggplot(data = fig2c,
              aes(x = genotype,
                  y = nr4a1)) +
  ...
  geom_line(aes(group = experiment_id),
            position = position_dodge(width = 0.2),
            color = "gray40") +
  geom_point(data = summary(lmm.ri_emm),
             aes(y = emmean,
                 color = genotype),
             size = 3) +
  geom_errorbar(data = summary(lmm.ri_emm),
                aes(y = emmean,
                    ymin = lower.CL,
                    ymax = upper.CL,
                    color = genotype),
...
### By modeling non-independence, the researchers gained free power
...
model_table |>
  ...
  pack_rows("lm1:  linear model", 2, 2)
```

Notes

1. The p-value for the linear model is 0.1426 while that of the random intercept model is 0.0217. Increased power will generally be the case with experimental bench biology data.
2. More specifically, compared to the linear model, the random intercept model has increased precision of the effect estimate (a smaller SE) BUT also decreased degrees of freedom (df). The increased precision will always be the case if the correlated error is positive.
...
### Better Know the linear mixed model

Intraclass correlation

```{r}
#| label: lmm-fig2c-varcorr
#| 
...
## Example 3: RCBD with > 2 groups (classical equivalent: repeated measures ANOVA) {#sec-lmm-example2}
...
```{r lmm-fig2c-right-import, echo=FALSE, message=FALSE}
data_from <- "NR4A transcription factors limit CAR T cell function in solid tumours"
file_name <- "41586_2019_985_MOESM8_ESM.xlsx"
file_path <- here(data_folder, data_from, file_name)

fig2c_r_import <- function(range_in = "xxx", response_in = ""){
  fig2c_wide <- read_excel(file_path,
                           sheet = "Fig 2",
                           range = range_in,
                           col_names = TRUE) |>
    data.table()
  genotype_levels <- c("PD1+TIM3+", "PD1+TIM3-", "PD1-TIM3-")
  colnames(fig2c_wide) <- genotype_levels
  fig2c_wide[, experiment_id := paste("exp_", .I)]
  fig2c_in <- melt(fig2c_wide,
                   id.vars = "experiment_id",
...
fig2c_r <- fig2c_r_import(range_in = "F3:H6", response_in = "nr4a1")
fig2c_r <- merge(fig2c_r, 
               fig2c_r_import(range_in = "F8:H11", response_in = "nr4a2"),
               by = c("genotype", "experiment_id"))
fig2c_r <- merge(fig2c_r, 
               fig2c_r_import(range_in = "F13:H16", response_in = "nr4a3"),
               by = c("genotype", "experiment_id"))
```
...
lmm.ri_pairs
```
### lmm.ce -- linear model for correlated error

```{r}
#| label: lmm-fig2c-r-lmm.ce
#| eval: false

lmm.ce <- lme(nr4a1 ~ genotype,
                random = ~1|experiment_id,
                correlation = corSymm(form = ~ 1 | experiment_id),
                weights = varIdent(form = ~ 1 | genotype),
                data = fig2c_r)
```

### rmaov.mul -- repeated measures ANOVA (RM-ANOVA), multivariate model
...
### rmaov.uni -- repeated measures ANOVA (RM-ANOVA), univariate model
...
1. lmm.ce fails.
2. rmaov.mul and ppttm are equivalent. This is generally true if there are no missing data (genotype:experiment_id combinations). Both rmaov.mul and ppttm require balance but ppttm is a pairwise procedure while rmaov.mul analyzes all treatment levels simultaneously. Consequently ppttm will exclude fewer measures and will have slightly more power than rmaov.mul. 
3. rmaov.mul and lmm.ri are not equivalent and this is generally true. 
4. rmaov.uni and lmm.ri are equivalent and this is generally true if there are no missing data.
5. The lmm equivalent of rmaov.mul is lmm.ris (random intercept and slopte model) but this model cannot be fit without replication with genotype:experiment_id combinations.
6. One reasonable conclusion of item 4 is that rmaov.mul effectively overfits the data and we should use rmaov.uni/lmm.ri as the model of choice. The simulation under RCBD above argues against this. Instead, rmaov.mul/lmm.ris is preferred (although lmm.ris cannot be fit without replication)
...
```{r}
#| label: lmm-fig2c-r-ggplot
#| echo: false

ggptm(rmaov.mul, rmaov.mul_emm, rmaov.mul_pairs,
              join_blocks = TRUE,
              block_id = "experiment_id",
      y_label = "MFI",
      palette = pal_okabe_ito,
      x_axis_labels = levels(fig2c_r$genotype))
```

```{r}
#| label: lmm-fig2c-r-ggplot-manual
#| echo: false
#| eval: false
#| 
...
1. The blocks are connected by the gray lines
2. The modeled SE of the treatment means are large relative to the spread of the treatment replicates. The modeled SE of the treatment means are not a good guide to differences among the treatments in a model with random factors if the sample size (number of blocks) is small.
...
model_table |>
  ...
  pack_rows("lm1:  linear model", 4, 6)
```

Notes

1. Only two of the three p-values are smaller for the RM-ANOVA model. Increased power will generally be the case with experimental bench biology data.
2. More specifically, compared to the linear model, the RM-ANOVA model has increased precision of the effect estimate (a smaller SE) BUT also decreased degrees of freedom (df). The increased precision will always be the case if the correlated error is positive.
...
## Example 4 -- RCBD with two factors (fig5c)
...
```{r lmm-fig5c-import, echo = FALSE, message=FALSE, warning=FALSE}
...
fig5c_wide <- read_excel(file_path,
                         sheet = "Fig5c",
                         range = "A2:M3",
                         col_names = FALSE) |>
  data.table() |>
...
treatment_levels <- names(fig5c_wide)[1:2]
fig5c_wide[, activity := rep(activity_levels, each = 6)]
fig5c_wide[, activity := factor(activity, levels = activity_levels)]

fig5c <- melt(fig5c_wide,
              id.vars = "activity",
              variable.name = "treatment",
              value.name = "glucose_uptake")
fig5c[, treatment := factor(treatment, levels = treatment_levels)]

fig5c[, donor := rep(paste0("donor_", 1:6), 4)]
```

### Understand the data

The data for Example 4 are from Figure 5c. Six muscle source cells were used to start six independent cultures. Cells from each culture were treated with either a negative control ("Scr") or a siRNA ("siNR4A3") that "silences" expression of the NR4A3 gene product by [cleaving the mRNA](https://en.wikipedia.org/wiki/Small_interfering_RNA){target="_blank"}. Glucose uptake in the two cell types was measured at rest ("Basal") and during electrical pulse stimulation ("EPS").
...
```{r lmm-fig5c-examine, echo = FALSE}
fig5c[, treatment_flat := paste(treatment, activity)]
treatment_flat_levels <- c("Scr Basal", "Scr EPS", "siNR4A3 Basal", "siNR4A3 EPS")
fig5c[, treatment_flat := factor(treatment_flat, levels = treatment_flat_levels)]

ggplot(data = fig5c,
       aes(x = treatment_flat,
           y = glucose_uptake,
           color = donor)) +
  geom_point() +
...
### Model fit and inference {#lmm-fig5c-m1}

```{r lmm-fig5c_m1, warning=FALSE, message=FALSE}
...
# check AIC
AIC(fig5c.ri, fig5c.rii2, fig5c.ce)

# check VarCorr model c
VarCorr(fig5c.rii2) # fine

# report fig5c_lmm2.rii (based on AIC and VarCorr check)
fig5c_m1 <- fig5c.ce
...
The boundary warnings indicate a variance component with no estimated variance, so these are excluded. There is trivial difference in AIC between fig5c_lmm.ri and fig5c_lmm2.rii and fig5c_ce <2 units from these. I recommend choosing to report the most maximal model (fig5c_ce or fig5c_rmaov.mul).


```{r, echo=FALSE, eval=FALSE}
fig5c.ri_emm <- emmeans(fig5c.ri, specs = c("treatment", "activity"))
fig5c.ri_pairs <- contrast(fig5c.ri_emm,
                               method = "revpairwise",
                               simple = "each",
                               combine = TRUE,
                               adjust = "none") |>
...
fig5c.rii2_emm <- emmeans(fig5c.rii2, specs = c("treatment", "activity"))
fig5c.rii2_pairs <- contrast(fig5c.rii2_emm,
                               method = "revpairwise",
                               simple = "each",
                               combine = TRUE,
                               adjust = "none") |>
...
fig5c.ce_emm <- emmeans(fig5c.ce, specs = c("treatment", "activity"))
fig5c.ce_pairs <- contrast(fig5c.ce_emm,
                               method = "revpairwise",
                               simple = "each",
                               combine = TRUE,
                               adjust = "none") |>
...
fig5c.rmaov_emm <- emmeans(fig5c.rmaov, specs = c("treatment", "activity"))
fig5c.rmaov_pairs <- contrast(fig5c.rmaov_emm,
                               method = "revpairwise",
                               simple = "each",
                               combine = TRUE,
                               adjust = "none") |>
...
```{r lmm-fig5c_emm}
fig5c_m1_emm <- emmeans(fig5c_m1, specs = c("treatment", "activity"))
```

```{r lmm-fig5c_emm-show, echo=FALSE}
fig5c_m1_emm |>
...
```{r lmm-fig5c-planned-show, echo=FALSE}
fig5c_m1_planned |>
  kable(digits = c(1,2,3,3,2,2,2,3)) |>
  kable_styling()
```

#### Plot the model {#lmm-fig5c-plotthemodel}

```{r echo=FALSE}
# add treatment column to emmeans table
fig5c_m1_emm_dt <- summary(fig5c_m1_emm) |>
  data.table()
fig5c_m1_emm_dt[, treatment_flat := paste(treatment, activity)]
fig5c_m1_emm_dt[, treatment_flat := factor(treatment_flat,
                                      levels = levels(fig5c$treatment_flat))]

# add group1 and group2 columns to exp6g_m1_planned
fig5c_m1_planned_dt <- data.table(fig5c_m1_planned)
fig5c_m1_planned_dt[, pretty_p := pvalString(p.value)]
fig5c_m1_planned_dt[, group1 := c("Scr EPS", "siNR4A3 EPS", "")]
fig5c_m1_planned_dt[, group2 := c("Scr Basal", "siNR4A3 Basal", "")]

```

```{r echo=FALSE}
fig5c_effects <- ggplot_the_effects(fig5c_m1_emm,
                   fig5c_m1_planned,
...
fig5c_response <- ggplot(data = fig5c,
              aes(x = treatment_flat,
                  y = glucose_uptake)) +
  ...
  geom_line(aes(group = donor),
            position = position_dodge(width = 0.2),
            color = "gray80") +
  geom_point(data = fig5c_m1_emm_dt,
             aes(y = emmean,
             color = activity),
             size = 3) +
  geom_errorbar(data = fig5c_m1_emm_dt,
             aes(y = emmean,
                 ymin = lower.CL,
                 ymax = upper.CL,
             color = activity),
...
fig5c_response <- factor_wrap(fig5c_response)

#fig5c_response
```

```{r}
#| label: fig-lmm-5c-effects
#| echo: false
#| warning: false
#| eval: true
#| fig-width: 8
#| fig-height: 4
#| fig-cap: "Treatment effect on glucose uptake. Gray dots connected by lines are individual donors."
#| 
plot_grid(fig5c_effects, fig5c_response,
          nrow=2,
          align = "v",
          axis = "lr",
          rel_heights = c(0.6,1))
...
```{r}
#| label: fig-lmm-fig5c-alternaplot
#| echo: false
#| warning: false
#| eval: true
#| fig-width: 6
#| fig-height: 3
#| fig-cap: "Glucose response to different treatments. Gray dots connected by lines are individual donors. Dashed gray line is expected additive mean of \"siNR4A3 EPS\"."
#| 

# get coefficients of model
b <- coef(summary(fig5c_m1))[, "Value"]

# get interaction p
p_ixn <- fig5c_m1_planned_dt[contrast == "Interaction", pretty_p] # check!

dodge_width <- 0.4

gg <- fig5c_response +
  # pvalue brackets
  stat_pvalue_manual(fig5c_m1_planned_dt[1:2],
                     label = "pretty_p",
...
```{r}
#| label: fig-lmm-fig5c-why
#| echo: false
#| warning: false
#| eval: true
#| fig-width: 6
#| fig-height: 3
#| fig-cap: "Why we care about blocking. The black dots are the modeled means of each treatment combination. The colored dots are the measured values of the response for each donor. The position of a donor relative to the mean is easy to see with these data."
#| 
...
m1_emm_dt <- emmeans(m1, specs = c("treatment", "activity")) |>
  summary() |>
  data.table()
m1_emm_dt[, treatment_flat:= paste(treatment, activity)]
m1_emm_dt[, treatment_flat:= factor(treatment_flat, treatment_flat)]
...
```{r lmm-fig5c-m2, echo=TRUE}
fig5c_m2 <- lm(glucose_uptake ~ treatment * activity,
           data = fig5c)

```

```{r lmm-fig5c_fixed, echo = FALSE}
fig5c_m2_emm <- emmeans(fig5c_m2, specs = c("treatment", "activity"))
fig5c_m2_planned <- contrast(fig5c_m2_emm,
                       method = list(
                         "(Scr EPS) - (Scr Basal)" = c(scr_eps - scr_basal),
                         "(siNR4A3 EPS) - (siNR4A3 Basal)" = c(siNR4A3_eps - siNR4A3_basal),
                         "Interaction" = c(siNR4A3_eps - siNR4A3_basal) -
...
```{r}
#| label: fig-lmm-fig5c-why2
#| echo: false
#| warning: false
#| eval: true
#| fig-width: 7
#| fig-height: 3
#| fig-cap: "A. Inference from a linear mixed model with blocking factor (donor) added as a random intercept. B. Inference from a fixed effects model."
#| 
gg1 <- ggplot_the_effects(fig5c_m1,
                 fig5c_m1_planned,
                 effect_label = "Difference in Glucose uptake\n(pmol per min)")

gg2 <- ggplot_the_effects(fig5c_m2,
                 fig5c_m2_planned,
                 effect_label = "Difference in Glucose uptake\n(pmol per min)")
...
@fig-lmm-fig5c-why2 A is a plot of the effects from the linear mixed model that models the correlated error due to donor. @fig-lmm-fig5c-why2 B is a plot of the effects from the fixed effect model that ignores the correlated error due to donor. Adding $\texttt{donor}$ as a factor to the linear model increases the precision of the estimate of the treatment effects by eliminating the among-donor component of variance from the error variance.

## Example 5 -- GRCBD Experiments with replication (exp1g) {#sec-lmm-example4}
...
#| fig-width: 8
#| fig-height: 4
#| fig-cap: "Treatment effect on germinal center (GC) formation. Small, pale, colored dots are independent experiments. Intermediate size colored dots are experiment means."
#| 
...
```{r lmm-exp1g-lm.agg-means-pool, echo=TRUE}
# means pooling model
...
```{r lmm-exp1g-lm.agg-planned, echo=FALSE}
...
Source: [Diabetes relief in mice by glucose-sensing insulin-secreting human α-cells](https://www.nature.com/articles/s41586-019-0942-8){target="_blank"}

[public source](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC6624841/){target="_blank"}

[data source](https://www.nature.com/articles/s41586-019-0942-8#Sec48){target="_blank"}
...
1. Factor 1 (Main plot): `treatment`
* αGFP -- the control
* αpm -- PDX1 + MAFA.
* αpm+HM -- αpm + HM. 
2. Factor 2 (Subplot) `glucose`
* low -- 3 mmol glucose
...
1. [Pdx1](https://en.wikipedia.org/wiki/PDX1) and [MafA](https://en.wikipedia.org/wiki/MAFA_(gene)) are transcription factors that stimulate differentiation of pancreatic alpha cells into insulin-secreting beta cells.
2. HM is a combination of human mesenchymal stem cells (MSCs) and human umbilical vein endothelial cells (HUVECs) and provide the appropriate environment for the alpha cells to produce insulin.
3. c-peptide is a stretch of amino-acids in between the alpha and beta subunits of insulin in the pro-insulin protein. Within the secretory vesicle, the c-peptide is cleaved leaving the alpha and beta subunits to form the insulin protein. c-peptide and insulin are secreted 1:1 so measures of c-peptide are markers of insulin secretion.
...
fig1f[, .(N = .N), by = .(treatment, glucose)]
```

### lmm.sp linear mixed model for split plot design
...
```{r}
rmaov.sp <- aov_4(c_peptide ~ treatment * glucose + (glucose | donor_id), data = fig1f)
rmaov.sp_emm <- emmeans(rmaov.sp, specs = c("treatment", "glucose"))
...
```{r}
pairs_dt <- rbind(lmm.sp_pairs, rmaov.sp_pairs)
...
pairs_dt |>
  ...
  pack_rows("rmaov.sp RMANOVA for split plot", 10, 18)
```

Notes

1. The design is balanced
2. Within the treatment factor, where there are only two levels of the other factor, inference is the same between the lmm.sp and rmaov.sp.
3. But, within the glucose factor, where there are three levels of the other factor, inference is different, because different SEs are computed for the Low vs. Hi contrasts. The rmaov.sp model is the more maximal model.

## So, which model, and why? A simulation to help decide
### Single factor RCBD models

With any design that creates non-independence, two linear mixed modeling strategies would be

1. Fit the maximal model first, if the model fails or has boundary warning because of too little variance at some level, then fit the next most maximal model, and so on. For a RCBD with >2 groups, this order would be lmm.ce, lmm.ri, lm.fe (lmm.ri and lm.fe are equivalent if the design is balanced).
2. Fit all linear mixed models and use a model selection method to choose the best model using a model selection method such as AIC.

I suggest a third strategy, which I prefer. This is,

3. use simulation to compare the performance (Type I error and Power) of each model under differ combinations of the structure of the variances of the levels of the random factor. Use these results to guide the strategy
...
1. High correlated error. There is heterogenous correlated error among the three pairs of treatments, with one high, one intermediate, and one low correlation
2. Low correlated error. There is heterogenous correlated error among the three pairs of treatments, all with low correlation.
3. Homogenous correlated error. There is high correlated errror among the three pairs of treatment, all with the same expected level
4. There is no correlated error.
...
error_type <- "power" 
if(error_type == "type1"){
  outfile_name <- "rcbd_type1.Rds"
  # parameters
  beta_i = c(10, 0, 0)
}
if(error_type == "power"){
    outfile_name <- "rcbd_power.Rds"
  # parameters
  beta_i = c(10, 0, 1.2)
}
...
model_levels <- c("lm.crd", "lm.fe", "lmm.ce", "rmaov.mul", "rmaov.uni1x", "ppttm")
...
if(do_sim == TRUE){
  for(param_set in 1:nrow(param_set_matrix)){
    ...
    for(i in 1:n_sims){
      ...
      # models
      form_lm.crd <- paste(paste0("sim_", i), "~ treatment") |>
        formula()
      form_lm.fe <- paste(paste0("sim_", i), "~ treatment + experiment") |>
        formula()
      form_lmm.ri <- paste(paste0("sim_", i), "~ treatment + (1 | experiment)") |>
        formula()
      form_pptt <- paste(paste0("sim_1"), "~ treatment + (1 | experiment)") |>
        formula()
      form_aov <- paste(paste0("sim_", i), "~ treatment + (treatment | experiment)") |>
        formula()
      lm.crd <- lm(form_lm.crd, data = fake_rcbd)
      lm.fe <- lm(form_lm.fe, data = fake_rcbd)
      # lmm.ri <- lmer(form_lmm.ri, data = fake_rcbd)
      lmm.ce <- lme(form_lm.crd,
                  random = ~1 | experiment,
                  correlation = corSymm(form = ~ 1 | experiment),
                  weights = varIdent(form = ~ 1 | treatment),
                  data = fake_rcbd) |>
        try()
      if(inherits(lmm.ce, "try-error")){
        lmm.ce <- NULL
      }
      
      # rmaov.uni used for both multivariate (rmaov.mul) and univariate (rmaov.uni)
      rmaov.uni <- aov_4(form_aov,
                    fun_aggregate = mean,
                    include_aov = TRUE,
                    data = fake_rcbd)
      ...
      lm.crd_pairs <- emmeans(lm.crd, specs = "treatment") |>
        contrast(method = "revpairwise", adjust = "none")
      lm.fe_pairs <- emmeans(lm.fe, specs = "treatment") |>
        contrast(method = "revpairwise", adjust = "none")
      rmaov.mul_pairs <- emmeans(rmaov.uni, specs = "treatment", model = "multivariate") |>
        contrast(method = "revpairwise", adjust = "none")
      rmaov.uni1x_pairs <- emmeans(rmaov.uni, specs = "treatment", model = "univariate") |>
        contrast(method = "revpairwise", adjust = "none")
      pptt_pairs <- pptt(form_pptt, data = fake_rcbd_means)
      
     
      fake_rcbd[, y_res := residuals(lm.crd)]
      ...
      lm.crd_p <- summary(lm.crd_pairs)[, "p.value"]
      lm.fe_p <- summary(lm.fe_pairs)[, "p.value"]
      rmaov.mul_p <- summary(rmaov.mul_pairs)[, "p.value"]
      rmaov.uni1x_p <- summary(rmaov.uni1x_pairs)[, "p.value"]
      pptt_p <- pptt_pairs[, p.value]
      
      p1_mat[i, 1] <- lm.crd_p[1]
      p1_mat[i, 2] <- lm.fe_p[1]
      p1_mat[i, 4] <- rmaov.mul_p[1]
      p1_mat[i, 5] <- rmaov.uni1x_p[1]
      p1_mat[i, 6] <- pptt_p[1]
      
      p2_mat[i, 1] <- lm.crd_p[2]
      p2_mat[i, 2] <- lm.fe_p[2]
      p2_mat[i, 4] <- rmaov.mul_p[2]
      p2_mat[i, 5] <- rmaov.uni1x_p[2]
      p2_mat[i, 6] <- pptt_p[2]
      
      p3_mat[i, 1] <- lm.crd_p[3]
      p3_mat[i, 2] <- lm.fe_p[3]
      p3_mat[i, 4] <- rmaov.mul_p[3]
      p3_mat[i, 5] <- rmaov.uni1x_p[3]
      p3_mat[i, 6] <- pptt_p[3]
      
      if(!is.null(lmm.ce)){
        lmm4_pairs <- emmeans(lmm.ce, specs = "treatment") |>
          contrast(method = "revpairwise", adjust = "none")
        p1_mat[i, 3] <- summary(lmm4_pairs)[1, "p.value"]
        p2_mat[i, 3] <- summary(lmm4_pairs)[2, "p.value"]
        p3_mat[i, 3] <- summary(lmm4_pairs)[3, "p.value"]
      }else{
        p1_mat[i, 3] <- NA
        p2_mat[i, 3] <- NA
        p3_mat[i, 3] <- NA
  ...
  if(!is.null(outfile_name)){saveRDS(object = big_table, file = save_file_path)}
}
```

```{r}
#| label: tbl-lmm-rcbd-type1
#| echo: false
#| tbl-cap: Type I error rate the RCBD statistical models under different levels of correlation structure among the observations. The Cor Error column is the average correlated error of the residuals fit by a simple linear model y ~ treatment. Model abbreviations as in text.

data_from <- "simulations/grcbds/data"
# power
outfile_name <- "rcbd_type1.Rds"
save_file_path <- here(data_from, outfile_name)
big_table <- readRDS(file = save_file_path)
model_levels <- c("lm.crd", "lm.fe", "lmm.ce", "rmaov.mul", "rmaov.uni1x", "ppttm")
...
for(param_set in 1:nrow(param_set_matrix)){
  ...
  out_table <- rbind(
    out_table,
    data.table(
      contrast = c("Tr1 - Cn", "Tr2 - Cn", "Tr2 - Tr1"),
...
1. The naive linear model, which doesn't model the non-independence, can be slightly to extremely conservative. There is only good Type I error behavior only when there is something close to zero correlated error.

Results to inform our decision about which model for non-independence

2. When there is zero correlated error, of the methods that model non-independence, all have reasonable type I error control. Perhaps lmm.ce is very slightly anticonservative (one of the error rates is 0.07).
3. When there is equal correlated error, of the methods that model non-independence, all have reasonable type I error control (lmm.ce is a very slightly anti-conservative).
4. When there is heterogenous correlated error, of the methods that model non-independence, only rmaov.mul/ppttm/lmm.ce have reasonable type I error control (again, lmm.ce is very slightly anticonservative). lmm.ri/rmaov.mul have liberal Type I control in contrasts with low correlated error and conservative Type I control in contrasts with high correlated error.
...
```{r}
#| label: tbl-lmm-rcbd-power
#| echo: false
#| tbl-cap: Power of the RCBD statistical models under different levels of correlation structure among the observations. Power is given for the (Tr2 - Cn) and (Tr2 - Tr1) contrasts. Type I error is given for the (Tr1 - Cn) contrast as the true effect is zero for this contrast. The Cor Error column is the average correlated error of the residuals fit by a simple linear model y ~ treatment. Model abbreviations as in text.
...
setnames(out_table,
         ...
         c("Contrast", "Cor Error", model_levels))
out_table |>
  kable(digits = c(1, 2, 3, 3, 3, 3, 3, 3)) |>
  kable_styling() |>
  pack_rows("sim 1 - high correlated error", 1, 3) |>
  pack_rows("sim 2 - low correlated error", 4, 6) |>
  pack_rows("sim 3 - equal correlated error", 7, 9) |>
...
1. The naive linear model (lm.fe) has less power than methods that model non-independence. lm.fe only has about the same power as methods that model non-independence if there is something close to zero correlated error. This is why we want to use models for non-independence when we have a blocked experimental design.

Results to inform our decision about which model for non-independence

2. When there is zero correlated error, of the methods that model non-independence, lmm.ce has slightly higher power than lmm.ri/rmaov.uni/lm.fe. And lmm.ri/rmaov.uni/lm.fe have slightly higher power than rmaov.mul/pptt.
3. When there is equal correlated error, of the methods that model non-independence, lmm.ce and lmm.ri/rmaov.uni/lm.fe have slightly higher power than rmaov.mul/pptt.
...
### Are these results robust for a 2 x 2 RCBD?

This simulation is similar to that above.

1. High correlated error. There is heterogenous treatment:block variance giving rise to heterogenous correlated error among the pairs of treatments, ranging from intermediate to high magnitude.
2. Low correlated error. There is heterogenous treatment:block variance giving rise to heterogenous correlated error among the pairs of treatments, ranging from small to intermediate magnitude.
3. Homogenous correlated error. There is no homogenous treatment:block variance giving rise to high correlated error among the pairs of treatment, all with the same expected level.
4. There is no among block or treatment:block variance, giving rise to zero correlated error.


```{r rcbd-performance-2x2, warning=FALSE, message=FALSE, error=FALSE, echo=FALSE}
seed_starter <- 0
n_sims <- 500
do_sim <- FALSE
...
error_type <- "power" 
if(error_type == "type1"){
  outfile_name <- "rcbd_type1-2x2.Rds"
  # parameters
  beta_i = c(10, 0, 0, 0)
}
if(error_type == "power"){
    outfile_name <- "rcbd_power-2x2.Rds"
  # parameters
  beta_i = c(10, 1, 0.6, 0)
}
save_file_path <- here(data_from, outfile_name)
...
fac1_levels <- c("CD", "HFD")
fac2_levels <- c("Sal", "Treat")
fac_table <- expand.grid(fac1_levels, fac2_levels)
treatment_levels <- paste(fac_table[, 1], fac_table[, 2])
model_levels <- c("lm.crd", "lm.fe", "lmm.rii", "lmm.ce", "rmaov.mul", "rmaov.uni2x", "rmaov.uni1x", "pptt")

p1_mat <- matrix(as.numeric(NA), nrow = n_sims, ncol = length(model_levels))
p2_mat <- matrix(as.numeric(NA), nrow = n_sims, ncol = length(model_levels))
p3_mat <- matrix(as.numeric(NA), nrow = n_sims, ncol = length(model_levels))
p4_mat <- matrix(as.numeric(NA), nrow = n_sims, ncol = length(model_levels))
r_mat <- matrix(as.numeric(NA), nrow = n_sims, ncol = 4)
...
# each value is different param_set
sigma_exp.block_list <- c(0.5, 0.25, 0.8, 0)

# each value is different param_set
sigma_ss_list <- c(.35, .45, .4, 1)
...
if(do_sim == TRUE){
  for(param_set in 1:nrow(param_set_matrix)){
    ...
    sigma_exp.block_i <- param_set_matrix[param_set, sigma_block]
    sigma_exp.block.fac1_i <- param_set_matrix[param_set, .SD, .SDcols = treatment_levels[1:2]] |> as.numeric()
    sigma_exp.block.fac2_i <- param_set_matrix[param_set, .SD, .SDcols = treatment_levels[3:4]] |> as.numeric()
    sigma_ss_i <- param_set_matrix[param_set, sigma_ss]
    sigma_rep_i <- 0.1
    sigma_exp.fac1_i <- 0
    sigma_exp.fac2_i <- 0
    sigma_exp_i <- 0
    n_treat_i <- length(fac1_levels)
    n_block_i <- 10
    n_rep_i <- 1
    n_ss_i <- 1
    fake_rcbd <- simulator_2(
      seed_i = seed_starter,
      n_sim = n_sims,
      n_factors = 2,
      n_levels = n_treat_i, # number of treatment levels
      n_block = n_block_i, # number of litters (blocks)
      n_rep = n_rep_i, # number of mice per litter:treatment
      n_ss = n_ss_i, # number of subsamples
      design = "rcbd",
      ...
      sigma_exp = sigma_exp_i, # sd among experiments
      sigma_exp.block = sigma_exp.block_i, # sd among exp:block (or block if n_exp = 1)
      sigma_exp.fac1 = sigma_exp.fac1_i, # sd among exp:treat
      sigma_exp.fac2 = sigma_exp.fac2_i, # sd among exp:treat
      sigma_exp.block.fac1 = sigma_exp.block.fac1_i, # sd among exp:block:treat
      sigma_exp.block.fac2 = sigma_exp.block.fac2_i, # sd among exp:block:treat
      sigma_rep = sigma_rep_i,
      sigma_ss = sigma_ss_i, # sd among subsamples within replication of treatment:block
      fac1_names = fac1_levels,
      fac2_names = fac2_levels,
      block_name = "litter",
      rep_name = "mouse",
      ss_name = "ss",
    )
    
#    fake_rcbd[, c("diet", "drug") := tstrsplit(treatment, "-")]
    fake_rcbd[, diet := fac1]
    fake_rcbd[, drug := fac2]
    fake_rcbd[, treatment := paste(diet, drug)]
    treatment_levels <- unique(fake_rcbd$treatment)
    fake_rcbd[, treatment := factor(treatment, levels = treatment_levels)]
    for(i in 1:n_sims){
      
      # models
      form_lm.crd <- paste(paste0("sim_", i), "~ diet * drug") |>
        formula()
      form_lm.fe <- paste(paste0("sim_", i), "~ diet * drug + litter") |>
        formula()
      form_lmm.ri <- paste(paste0("sim_", i), "~ diet * drug + (1 | litter)") |>
        formula()
      form_lmm.rii <- paste(paste0("sim_", i), "~ diet * drug + (1 | litter) + (1 | litter:diet)  + (1 | litter:drug)") |>
        formula()
      form_pptt <- paste(paste0("sim_1"), "~ treatment + (1 | litter)") |>
        formula()
      form_aov <- paste(paste0("sim_", i), "~ diet * drug + (diet * drug | litter)") |>
        formula()
      form_aov_flat <- paste(paste0("sim_", i), "~ treatment + (treatment | litter)") |>
        formula()
      ...
      lm.crd <- lm(form_lm.crd, data = fake_rcbd)
      lm.fe <- lm(form_lm.fe, data = fake_rcbd)
      lmm.rii <- lmer(form_lmm.rii, data = fake_rcbd)
      lmm.ce <- lme(form_lm.crd,
                  random = ~1 | litter,
                  correlation = corCompSymm(form = ~ 1 | litter),
                  weights = varIdent(form = ~ 1 | treatment),
                  data = fake_rcbd) |>
        try()
      if(inherits(lmm.ce, "try-error")){
      ...
      # rmaov.uni used for both multivariate (rmaov.mul) and univariate (rmaov.uni)
      rmaov.mul <- aov_4(form_aov,
                    fun_aggregate = mean,
                    include_aov = TRUE,
                    data = fake_rcbd)
      rmaov.uni1x <- aov_4(form_aov_flat,
                      include_aov = TRUE,
                      data = fake_rcbd)
      
      lm.crd_pairs <- emmeans(lm.crd, specs = c("diet", "drug")) |>
        contrast(method = "revpairwise", simple = "each", combine = TRUE, adjust = "none")
      lm.fe_pairs <- emmeans(lm.fe, specs = c("diet", "drug")) |>
        contrast(method = "revpairwise", simple = "each", combine = TRUE, adjust = "none")
      lmm.rii_pairs <- emmeans(lmm.rii, specs = c("diet", "drug")) |>
        contrast(method = "revpairwise", simple = "each", combine = TRUE, adjust = "none")
      rmaov.mul_pairs <- emmeans(rmaov.mul, specs = c("diet", "drug"), model = "multivariate") |>
        contrast(method = "revpairwise", simple = "each", combine = TRUE, adjust = "none")
      rmaov.uni2x_pairs <- emmeans(rmaov.mul, specs = c("diet", "drug"), model = "univariate") |>
        contrast(method = "revpairwise", simple = "each", combine = TRUE, adjust = "none")
      rmaov.uni1x_pairs <- emmeans(rmaov.uni1x, specs = c("treatment"), model = "univariate") |>
        contrast(method = "revpairwise", simple = "each", combine = TRUE, adjust = "none")
      ...
      fake_rcbd[, y_res := residuals(lm.crd)]
      fake_rcbd_wide <- dcast(fake_rcbd,
                              litter + rep ~ treatment,
                              value.var = "y_res")
      residual_correlations <- cor(fake_rcbd_wide[, .SD, .SDcols = treatment_levels])
      r_mat[i, 1] <- residual_correlations[2, 1]
      r_mat[i, 2] <- residual_correlations[4, 3]
      r_mat[i, 3] <- residual_correlations[3, 1]
      r_mat[i, 4] <- residual_correlations[4, 2]

      lm.crd_p <- summary(lm.crd_pairs)[, "p.value"]
      lm.fe_p <- summary(lm.fe_pairs)[, "p.value"]
      lmm.rii_p <- summary(lmm.rii_pairs)[, "p.value"]
      rmaov.mul_p <- summary(rmaov.mul_pairs)[, "p.value"]
      rmaov.uni2x_p <- summary(rmaov.uni2x_pairs)[, "p.value"]
      rmaov.uni1x_p <- summary(rmaov.uni1x_pairs)[, "p.value"]
      pptt_p <- pptt_pairs[, p.value]
     
      p1_mat[i, 1] <- lm.crd_p[1]
      p1_mat[i, 2] <- lm.fe_p[1]
      p1_mat[i, 3] <- lmm.rii_p[1]
      p1_mat[i, 5] <- rmaov.mul_p[1]
      p1_mat[i, 6] <- rmaov.uni2x_p[1]
      p1_mat[i, 7] <- rmaov.uni1x_p[1]
      p1_mat[i, 8] <- pptt_p[1]
      
      p2_mat[i, 1] <- lm.crd_p[2]
      p2_mat[i, 2] <- lm.fe_p[2]
      p2_mat[i, 3] <- lmm.rii_p[2]
      p2_mat[i, 5] <- rmaov.mul_p[2]
      p2_mat[i, 6] <- rmaov.uni2x_p[2]
      p2_mat[i, 7] <- rmaov.uni1x_p[2]
      p2_mat[i, 8] <- pptt_p[6]
      
      p3_mat[i, 1] <- lm.crd_p[3]
      p3_mat[i, 2] <- lm.fe_p[3]
      p3_mat[i, 3] <- lmm.rii_p[3]
      p3_mat[i, 5] <- rmaov.mul_p[3]
      p3_mat[i, 6] <- rmaov.uni2x_p[3]
      p3_mat[i, 7] <- rmaov.uni1x_p[3]
      p3_mat[i, 8] <- pptt_p[2]

      p4_mat[i, 1] <- lm.crd_p[4]
      p4_mat[i, 2] <- lm.fe_p[4]
      p4_mat[i, 3] <- lmm.rii_p[4]
      p4_mat[i, 5] <- rmaov.mul_p[4]
      p4_mat[i, 6] <- rmaov.uni2x_p[4]
      p4_mat[i, 7] <- rmaov.uni1x_p[4]
      p4_mat[i, 8] <- pptt_p[5]
      
      if(!is.null(lmm.ce)){
        # lmm4_pairs <- emmeans(lmm.ce, specs = c("diet", "drug"), mode = "satterthwaite") |>
        #   contrast(method = "revpairwise", simple = "each", combine = TRUE, adjust = "none")
        lm.ce_pairs <- emmeans(lmm.ce, specs = c("diet", "drug")) |>
          contrast(method = "revpairwise", simple = "each", combine = TRUE, adjust = "none")
        p1_mat[i, 4] <- summary(lm.ce_pairs)[1, "p.value"]
        p2_mat[i, 4] <- summary(lm.ce_pairs)[2, "p.value"]
        p3_mat[i, 4] <- summary(lm.ce_pairs)[3, "p.value"]
        p4_mat[i, 4] <- summary(lm.ce_pairs)[4, "p.value"]
      }else{
        p1_mat[i, 4] <- NA
        p2_mat[i, 4] <- NA
        p3_mat[i, 4] <- NA
    ...
    sim_table <- cbind(r_mat, p1_mat, p2_mat, p3_mat, p4_mat)
    contrast_names <- c("HFD Sal - CD Sal", "HFD Treat - CD Treat",
                                   "CD Treat - CD Sal", "HFD Treat - HFD Sal")
    sim_table_names <- paste(rep(model_levels, 3),
                             rep(contrast_names,
                                 each = length(model_levels)),
                             sep = "_")
    colnames(sim_table) <- c("r11", "r12", "r21", "r22", sim_table_names)
    ...
    big_table <- rbind(
      ...
      data.table(
        ...
        n_ss = n_ss_i,
        sigma_block = sigma_exp.block_i,
        sigma_block.fac1_1 = sigma_exp.block.fac1_i[1],
        sigma_block.fac1_2 = sigma_exp.block.fac1_i[2],
        sigma_block.fac2_1 = sigma_exp.block.fac2_i[1],
        sigma_block.fac2_2 = sigma_exp.block.fac2_i[2],
        sigma_exp.treat = sigma_exp.fac1_i,
        sigma_rep = sigma_rep_i,
        sigma_exp = sigma_exp_i,
        sigma_ss = sigma_ss_i,
        sim_table
...
out_table <- data.table(NULL)
for(param_set in 1:nrow(param_set_matrix)){
  r_mat <- big_table[sim_id == param_set, .SD, .SDcols = c("r11", "r12", "r21", "r22")]
  p1_mat <- big_table[sim_id == param_set, .SD,
                      .SDcols = paste0(model_levels, "_", contrast_names[1])]
  p2_mat <- big_table[sim_id == param_set, .SD,
                      .SDcols = paste0(model_levels, "_",  contrast_names[2])]
  p3_mat <- big_table[sim_id == param_set, .SD,
                      .SDcols = paste0(model_levels, "_",  contrast_names[3])]
  p4_mat <- big_table[sim_id == param_set, .SD,
                      .SDcols = paste0(model_levels, "_",  contrast_names[4])]
  out_table <- rbind(
    out_table,
    data.table(
...
setnames(out_table,
         ...
         c("Contrast", "Cor Error", model_levels))
out_table |>
  kable(digits = c(1, 3, 3, 3, 3, 3, 3, 3, 3, 3)) |>
  kable_styling() |>
  pack_rows("sim 1 - high correlated error", 1, 4) |>
  pack_rows("sim 2 - low correlated error", 5, 8) |>
  pack_rows("sim 3 - equal correlated error", 9, 12) |>
  pack_rows("sim 4 - zero correlated error", 13, 16)
...
2. When there is zero correlated error, of the methods that model non-independence, all have reasonable type I error control, except lmm.ce, which is anticonservative.
3. When there is equal correlated error, of the methods that model non-independence, all have reasonable type I error control, except lmm.ce, which is anticonservative.
...
```{r}
#| label: tbl-lmm-rcbd-power-2x2
#| echo: false
#| tbl-cap: Power of the RCBD statistical models under different levels of correlation structure among the observations. The Cor Error column is the average correlated error of the residuals fit by a simple linear model y ~ treatment. Model abbreviations as in text.
...
for(param_set in 1:nrow(param_set_matrix)){
  ...
  inc <- which(!is.na(p1_mat[, 7]))
  out_table <- rbind(
    out_table,
    data.table(
      contrast = contrast_names,
      cor = apply(r_mat, 2, mean),
      rbind(apply(p1_mat[inc,], 2, pless),
            apply(p2_mat[inc,], 2, pless),
            apply(p3_mat[inc,], 2, pless),
            apply(p4_mat[inc,], 2, pless))
    ))
}
...
```{r rcbds-performance-sim, warning=FALSE, message=FALSE, error=FALSE, echo = FALSE}
seed_starter <- 0
n_sims <- 500
do_sim <- FALSE
...
error_type <- "type1" 
if(error_type == "type1"){
  outfile_name <- "rcbds_type1.Rds"
  # parameters
  beta_i = c(10, 0, 0)
}
if(error_type == "power"){
    outfile_name <- "rcbds_power.Rds"
  # parameters
  beta_i = c(10, 0, 1.2)
}
save_file_path <- here(data_from, outfile_name)
...
treatment_levels <- c("Cn", "Tr1", "Tr2")
model_levels <- c("lm.crd", "lm.fe", "lmm.ri", "lmm.ris", "lmm.ce", 
                  "rmaov.mul", "rmaov.uni1x", "ppttm")
...
if(do_sim == TRUE){
  for(param_set in 1:nrow(param_set_matrix)){
    ...
    for(i in 1:n_sims){
      ...
      # models
      form_lm.crd <- paste(paste0("sim_1"), "~ treatment") |>
        formula()
      form_lm.fe <- paste(paste0("sim_1"), "~ treatment + experiment") |>
        formula()
      form_lmm.ri <- paste(paste0("sim_", i), "~ treatment + (1 | experiment)") |>
        formula()
      form_lmm.ris <- paste(paste0("sim_", i), "~ treatment + (treatment | experiment)") |>
        formula()
      form_pptt <- paste(paste0("sim_1"), "~ treatment + (1 | experiment)") |>
        formula()
      form_aov <- paste(paste0("sim_", i), "~ treatment + (treatment | experiment)") |>
      ...
      lm.crd <- lm(form_lm.crd, data = fake_rcbd_means)
      lm.fe <- lm(form_lm.fe, data = fake_rcbd_means)
      lmm.ri <- lmer(form_lmm.ri, data = fake_rcbd)
      lmm.ris <- lmer(form_lmm.ris, data = fake_rcbd)
      # lmm.ce <- lme(form_lm.crd,
      #             random = ~1 | experiment,
      #             correlation = corSymm(form = ~ 1 | experiment),
      #             weights = varIdent(form = ~ 1 | treatment),
      ...
      fake_rcbd_means[, y_res := residuals(lm.crd)]
      fake_rcbd_wide <- dcast(fake_rcbd_means,
      ...
      lm.crd_p <- summary(lm.crd_pairs)[, "p.value"]
      lm.fe_p <- summary(lm.fe_pairs)[, "p.value"]
      lmm.ri_p <- summary(lmm.ri_pairs)[, "p.value"]
      lmm.ris_p <- summary(lmm.ris_pairs)[, "p.value"]
      rmaov.mul_p <- summary(rmaov.mul_pairs)[, "p.value"]
      rmaov.uni1x_p <- summary(rmaov.uni1x_pairs)[, "p.value"]
      pptt_p <- pptt_pairs[, p.value]
      ...
      p1_mat[i, 1] <- lm.crd_p[1]
      p1_mat[i, 2] <- lm.fe_p[1]
      p1_mat[i, 3] <- lmm.ri_p[1]
      p1_mat[i, 4] <- lmm.ris_p[1]
      p1_mat[i, 5] <- NA
      p1_mat[i, 6] <- rmaov.mul_p[1]
      p1_mat[i, 7] <- rmaov.uni1x_p[1]
      p1_mat[i, 8] <- pptt_p[1]
      
      p2_mat[i, 1] <- lm.crd_p[2]
      p2_mat[i, 2] <- lm.fe_p[2]
      p2_mat[i, 3] <- lmm.ri_p[2]
      p2_mat[i, 4] <- lmm.ris_p[2]
      p2_mat[i, 5] <- NA
      p2_mat[i, 6] <- rmaov.mul_p[2]
      p2_mat[i, 7] <- rmaov.uni1x_p[2]
      p2_mat[i, 8] <- pptt_p[2]
      
      p3_mat[i, 1] <- lm.crd_p[3]
      p3_mat[i, 2] <- lm.fe_p[3]
      p3_mat[i, 3] <- lmm.ri_p[3]
      p3_mat[i, 4] <- lmm.ris_p[3]
      p3_mat[i, 5] <- NA
      p3_mat[i, 6] <- rmaov.mul_p[3]
      p3_mat[i, 7] <- rmaov.uni1x_p[3]
      p3_mat[i, 8] <- pptt_p[3]
...
2. If the number of treatments > 2 
...
#### Models without subsampling - Example 3 (Experiment 2c right panel)
...
### Repeated measures ANOVA (randomized complete block with no subsampling) {#sec-lmm-rmanova}
...
# fig5c_aov included the univariate model but the default is still the multivariate model
...
1. If our rmANOVA model is fit with the default specification (`fig5c_rmaov.mul`), we can get the multivariate output using the `model = "multivariate"` argument within `emmeans` (not the `contrast` function!). Or, if our fit excluded the univariate model (`fig5c_aov1`), then we don't need the `model` argument.
...
1. Passing `fig5c_rmaov.mul` to `emmeans` will return the contrasts from the multivariate. change this to the univariate model by passing `fig5c_rmaov.mul$aov`.

#### The ANOVA table

```{r}
nice(fig5c_rmaov.mul, correction = "GG")
nice(fig5c_rmaov.mul, correction = "none")
```
...
if(doit == TRUE){
  ...
  lm.crd_p <- matrix(as.numeric(NA), nrow = n_sim, ncol = n_pairs)
  lm.fe_p <- matrix(as.numeric(NA), nrow = n_sim, ncol = n_pairs)
  lmm.ri_p <- matrix(as.numeric(NA), nrow = n_sim, ncol = n_pairs)
  lmm3_p <- matrix(as.numeric(NA), nrow = n_sim, ncol = n_pairs)
  lmm2_p <- matrix(as.numeric(NA), nrow = n_sim, ncol = n_pairs)
  ...
  for(i in 1:n_sim){
    ...
    lm.crd <- lm(y ~ tr1_main * tr2_sub,
              data = fake_i)
    lm.fe <- lm(y ~ tr1_main * tr2_sub + block + tr1_main:block,
              data = fake_i)
    lmm.ri <- lmer(y ~ tr1_main * tr2_sub + (1 | block),
                 data = fake_i)
    lmm.ris <- lmer(y ~ tr1_main * tr2_sub + (tr1_main|block),
                 data = fake_i)
    lmm.rii <- lmer(y ~ tr1_main * tr2_sub + (1 | block) + (1 | block:tr1_main),
                 data = fake_i)
    lmm5 <- lmer(y ~ tr1_main * tr2_sub + (1 | block) + (1 | block:tr2_sub),
                 data = fake_i)
    lmm.sp1 <- lmer(y ~ tr1_main * tr2_sub + (1 | block) + (1 | block:tr2_sub) + (1 | block:tr1_main),
                 data = fake_i)
    # lmm5 <- lmer(y ~ tr1_main * tr2_sub + block + (1 | block:tr1_main),
    #              data = fake_data)
    
    # rmaov.mul <- aov_4(y ~ tr1_main * tr2_sub + (tr1_main | block),
    #               data = fake_i)
    # 
    lm.crd_pairs <- emmeans(lm.crd, specs = c("tr1_main", "tr2_sub")) |>
      contrast(method = "revpairwise",
               simple = "each",
               combine = TRUE,
               adjust = "none") |>
      summary(infer = TRUE)
    lm.crd_p[i,] <- lm.crd_pairs[, "p.value"]
    
    lm.fe_pairs <- emmeans(lm.fe, specs = c("tr1_main", "tr2_sub")) |>
      contrast(method = "revpairwise",
               simple = "each",
               combine = TRUE,
               adjust = "none") |>
      summary(infer = TRUE)
    lm.fe_p[i,] <- lm.fe_pairs[, "p.value"]
    
    lmm.ri_pairs <- emmeans(lmm.ri, specs = c("tr1_main", "tr2_sub")) |>
      contrast(method = "revpairwise",
               simple = "each",
               combine = TRUE,
               adjust = "none") |>
      summary(infer = TRUE)
    lmm.ri_p[i,] <- lmm.ri_pairs[, "p.value"]
    
    lmm3_pairs <- emmeans(lmm.ris, specs = c("tr1_main", "tr2_sub")) |>
      contrast(method = "revpairwise",
               simple = "each",
               combine = TRUE,
               adjust = "none") |>
    ...
    lmm2_pairs <- emmeans(lmm.rii, specs = c("tr1_main", "tr2_sub")) |>
      contrast(method = "revpairwise",
               simple = "each",
               combine = TRUE,
               adjust = "none") |>
      summary(infer = TRUE)
    lmm2_p[i,] <- lmm2_pairs[, "p.value"]
    ...
    lmm6_pairs <- emmeans(lmm.sp1, specs = c("tr1_main", "tr2_sub")) |>
      contrast(method = "revpairwise",
               simple = "each",
               combine = TRUE,
               adjust = "none") |>
    ...
    # get checks
    #  lmm3_check[i] <- is.null(lmer_check(lmm.ris))
    lmm2_check[i] <- is.null(lmer_check(lmm.rii))
  ...
  big_table <- data.table(NULL)
  model_labels <- c("lm.crd", "lm.fe", "lmm.ri", "lmm.ris", "lmm.rii", "lmm5", "lmm.sp1", "r_mat")
  models <- list(lm.crd_p, lm.fe_p, lmm.ri_p, lmm3_p, lmm2_p, lmm5_p, lmm6_p, r_mat)
  ...
  data.table(
    ...
    lm.crd = apply(lm.crd_p, 2, pless),
    lm.fe = apply(lm.fe_p, 2, pless),
    lmm.ri = apply(lmm.ri_p, 2, pless),
    lmm.ris = apply(lmm3_p, 2, pless),
    lmm.rii = apply(lmm2_p, 2, pless),
    lmm5 = apply(lmm5_p, 2, pless),
    lmm.sp1 = apply(lmm5_p, 2, pless)
  )
  ...
  # 
  # data.table(
  #   all = apply(lmm2_p, 2, pless),
  #   fail = apply(lmm2_p[lmm3_check == FALSE, ], 2, pless),
  #   pass = apply(lmm2_p[lmm3_check == TRUE,], 2, pless)
  # )
...
if(readit == TRUE){
  ...
  for(sim_id_i in 1:4){
    ...
    lm.crd_p <- big_table[model == "lm.crd", ]
    lm.fe_p <- big_table[model == "lm.fe", ]
    lmm.ri_p <- big_table[model == "lmm.ri", ]
    lmm3_p <- big_table[model == "lmm.ris", ]
    lmm2_p <- big_table[model == "lmm.rii", ]
    lmm5_p <- big_table[model == "lmm5", ]
    lmm6_p <- big_table[model == "lmm.sp1", ]
    out_table <- data.table(
      contrast = rep(c("main", "sub"), c(3,6)),
      r = apply(r_mat[, .SD, .SDcols = p_cols], 2, mean),
      lm.crd = apply(lm.crd_p[, .SD, .SDcols = p_cols], 2, pless),
      lm.fe = apply(lm.fe_p[, .SD, .SDcols = p_cols], 2, pless),
      lmm.ri = apply(lmm.ri_p[, .SD, .SDcols = p_cols], 2, pless),
      lmm.ris = apply(lmm3_p[, .SD, .SDcols = p_cols], 2, pless),
      lmm.rii = apply(lmm2_p[, .SD, .SDcols = p_cols], 2, pless),
      lmm5 = apply(lmm5_p[, .SD, .SDcols = p_cols], 2, pless),
      lmm.sp1 = apply(lmm6_p[, .SD, .SDcols = p_cols], 2, pless)
    )
    summary_i <- out_table[, .(r = mean(r),
                               lm.crd = mean(lm.crd),
                               lm.fe = mean(lm.fe),
                               lmm.ri = mean(lmm.ri),
                               lmm.ris = mean(lmm.ris),
                               lmm.rii = mean(lmm.rii),
                               lmm5 = mean(lmm5),
                               lmm.sp1 = mean(lmm.sp1)),
                           by = .(contrast)]
    res_out <- rbind(
      res_out,
      data.table(
...
```{r lmm-fig5c-import-show, echo = TRUE}
data_from <- "Transcriptomic profiling of skeletal muscle adaptations to exercise and inactivity"
file_name <- "41467_2019_13869_MOESM6_ESM.xlsx"
file_path <- here(data_folder, data_from, file_name)
...
activity_levels <- c("Basal", "EPS")
treatment_levels <- names(fig5c_wide)
fig5c_wide[, activity := rep(activity_levels, each = 6)]
fig5c_wide[, activity := factor(activity, levels = activity_levels)]
