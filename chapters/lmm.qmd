...
1. Fit the most complete model first, if the model fails or has boundary warning because of too little variance at some level, then fit the next most complete model, and so on. This order would be lmm.ce, lmm.ri, lm.fe (lmm.ri and lm.fe are equivalent if the design is balanced).
2. Fit all linear mixed models (lmm.ce, lmm.ri) and use a model selection method to choose the best model that balances completeness (controling type I error) and overfitting.
...
model_levels <- c("lm.crd", "lm.fe", "rmaov.mul", "rmaov.uni1", "ppttm", "lmm.ce")
  ...
  for(param_set in 1:nrow(param_set_matrix)){
    ...
    for(i in 1:n_sims){
      ...
      rmaov.uni1x_pairs <- emmeans(rmaov.uni, specs = "treatment", model = "univariate") |>
        contrast(method = "revpairwise", adjust = "none")
      ppttm_pairs <- pptt(form_pptt, data = fake_rcbd_means)
      ...
      p1_mat[i, 3] <- rmaov.mul_p[1]
      p1_mat[i, 4] <- rmaov.uni1x_p[1]
      p1_mat[i, 5] <- pptt_p[1]
      
      p2_mat[i, 1] <- lm.crd_p[2]
      p2_mat[i, 2] <- lm.fe_p[2]
      p2_mat[i, 3] <- rmaov.mul_p[2]
      p2_mat[i, 4] <- rmaov.uni1x_p[2]
      p2_mat[i, 5] <- pptt_p[2]
      
      p3_mat[i, 1] <- lm.crd_p[3]
      p3_mat[i, 2] <- lm.fe_p[3]
      p3_mat[i, 3] <- rmaov.mul_p[3]
      p3_mat[i, 4] <- rmaov.uni1x_p[3]
      p3_mat[i, 5] <- pptt_p[3]
      
      if(!is.null(lmm.ce)){
        lmm4_pairs <- emmeans(lmm.ce, specs = "treatment") |>
          contrast(method = "revpairwise", adjust = "none")
        p1_mat[i, 6] <- summary(lmm4_pairs)[1, "p.value"]
        p2_mat[i, 6] <- summary(lmm4_pairs)[2, "p.value"]
        p3_mat[i, 6] <- summary(lmm4_pairs)[3, "p.value"]
      }else{
        p1_mat[i, 6] <- lm.fe_p[1]
        p2_mat[i, 6] <- lm.fe_p[2]
        p3_mat[i, 6] <- lm.fe_p[3]
      }
...
#### Are these results robust for a 2 x 2 design?
