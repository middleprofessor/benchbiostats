...
plot_response(rmaov.mul, rmaov.mul_emm, rmaov.mul_pairs,
              join_blocks = TRUE,
              block_id = "experiment_id")
```
...
## Example 4 -- RCBD with two factors (exp5c)
...
exp5c_rmaov.mul <- aov_4(glucose_uptake ~ treatment * activity +
...
exp5c_m1b is singular fit. don't use. Trivial difference in AIC between exp5c_m1a and exp5c_m1c. Nothing in VarCorr with exp5c_m1c raises red flags. Report exp5c_m1c.
...
* αGFP -- the control (gfp inserted via virus)
* αpm -- PDX1 + MAFA (inserted via virus)
* αpm+HM -- αpm + HM
...
1. [Pdx1](https://en.wikipedia.org/wiki/PDX1) and [MafA](https://en.wikipedia.org/wiki/MAFA_(gene)) are transcription factors that stimulate differentiation of pancreatic cells into insulin-secreting beta cells.
2. HM is a combination of human mesenchymal stem cells (MSCs) and human umbilical vein endothelial cells (HUVECs)
...
1. If our rmANOVA model is fit with the default specification (`exp5c_rmaov.mul`), we can get the multivariate output using the `model = "multivariate"` argument within `emmeans` (not the `contrast` function!). Or, if our fit excluded the univariate model (`exp5c_aov1`), then we don't need the `model` argument.

**contrasts from univariate model**

```{r}
# get the univariate model results using $aov
emmeans(exp5c_rmaov.mul$aov, specs = c("treatment", "activity")) |>
  contrast(method = "revpairwise",
           simple = "each",
           combine = TRUE,
           adjust = "none")
...
1. Passing `exp5c_rmaov.mul` to `emmeans` will return the contrasts from the multivariate. change this to the univariate model by passing `exp5c_rmaov.mul$aov`.

#### The ANOVA table

```{r}
nice(exp5c_rmaov.mul, correction = "GG")
nice(exp5c_rmaov.mul, correction = "none")
```
