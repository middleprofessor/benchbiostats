...
rmaov.uni1 <- aov_4(y ~ treatment + (treatment | block),
           include_aov = TRUE,
             data = figx)
# requires the argument model = "univariate" in the emmeans function
```
...
```{r, class.source = "fold-show", eval=FALSE}
rmaov.uni2 <- aov_4(y ~ factor1 * factor2 + (factor1 * factor2 | block),
           include_aov = TRUE,
             data = figx)
# requires the argument model = "univariate" in the emmeans function
```
...
1. These are numerically equivalent and should result in the same estimates, SE, CIs, and p-values
...
### Why we care about modeling batch in fig5c
  ...
  for(param_set in 1:nrow(param_set_matrix)){
    ...
    for(i in 1:n_sims){
      ...
      rmaov.mul_pairs <- emmeans(rmaov.uni, specs = "treatment", model = "multivariate") |>
        contrast(method = "revpairwise", adjust = "none")
      aov1_pairs <- emmeans(rmaov.uni, specs = "treatment", model = "univariate") |>
        contrast(method = "revpairwise", adjust = "none")
...
model_levels <- c("lm.crd", "lm.fe", "lmm.rii", "rmaov.mul", "rmaov.uni", "rmaov.uni1", "pptt", "lmm.ce")
...
#    fake_rcbd[, c("diet", "drug") := tstrsplit(treatment, "-")]
    ...
    for(i in 1:n_sims){
      ...
      form_lmm.ri <- paste(paste0("sim_", i), "~ diet * drug + (1 | litter)") |>
        formula()
      form_lmm2 <- paste(paste0("sim_", i), "~ diet * drug + (1 | litter) + (1 | litter:diet)  + (1 | litter:drug)") |>
        formula()
 ...
 #     lmm.ri <- lmer(form_lmm.ri, data = fake_rcbd)
      lmm.ce <- lme(form_lm.crd,
                  ...
                  weights = varIdent(form = ~ 1 | diet * drug),
                  data = fake_rcbd) |>
        try()
      if(inherits(lmm.ce, "try-error")){
        lmm.ce <- NULL
      ...
      # lmm.rii <- lmer(form_lmm2, data = fake_rcbd) |>
      #   try()
      # if(inherits(lmm.ce, "try-error")){
      #   lmm.rii <- NULL
      # }
      ...
      rmaov.uni <- aov_4(form_aov,
                    ...
                    include_aov = TRUE,
                    data = fake_rcbd)
      rmaov.uni1 <- aov_4(form_aov_flat,
                      include_aov = TRUE,
                      data = fake_rcbd)
      ...
      lm.fe_pairs <- emmeans(lm.fe, specs = c("diet", "drug")) |>
        contrast(method = "revpairwise", simple = "each", combine = TRUE, adjust = "none")
      # lmm.ri_pairs <- emmeans(lmm.ri, specs = c("diet", "drug")) |>
      #   contrast(method = "revpairwise", simple = "each", combine = TRUE, adjust = "none")
      # lmm2_pairs <- emmeans(lmm.rii, specs = c("diet", "drug")) |>
      #   contrast(method = "revpairwise", simple = "each", combine = TRUE, adjust = "none")
      rmaov.mul_pairs <- emmeans(rmaov.uni, specs = c("diet", "drug"), model = "multivariate") |>
        contrast(method = "revpairwise", simple = "each", combine = TRUE, adjust = "none")
      aov1_pairs <- emmeans(rmaov.uni, specs = c("diet", "drug"), model = "univariate") |>
        contrast(method = "revpairwise", simple = "each", combine = TRUE, adjust = "none")
      rmaov.uni1_pairs <- emmeans(rmaov.uni1, specs = c("treatment"), model = "univariate") |>
        contrast(method = "revpairwise", simple = "each", combine = TRUE, adjust = "none")
      ...
      rmaov.mul_p <- summary(rmaov.mul_pairs)[, "p.value"]
      aov1_p <- summary(aov1_pairs)[, "p.value"]
      rmaov.uni1_p <- summary(rmaov.uni1_pairs)[, "p.value"]
      pptt_p <- pptt_pairs[, p.value]
      ...
      p3_mat[i, 3] <- rmaov.mul_p[3]
      p3_mat[i, 4] <- aov1_p[3]
      p3_mat[i, 5] <- rmaov.uni1_p[2]
      p3_mat[i, 6] <- pptt_p[2]

      p4_mat[i, 1] <- lm.crd_p[4]
      p4_mat[i, 2] <- lm.fe_p[4]
      p4_mat[i, 3] <- rmaov.mul_p[4]
      p4_mat[i, 4] <- aov1_p[4]
      p4_mat[i, 5] <- rmaov.uni1_p[5]
      p4_mat[i, 6] <- pptt_p[5]
      
      if(!is.null(lmm.ce)){
        # lmm4_pairs <- emmeans(lmm.ce, specs = c("diet", "drug"), mode = "satterthwaite") |>
        #   contrast(method = "revpairwise", simple = "each", combine = TRUE, adjust = "none")
        lmm4_pairs <- emmeans(lmm.ce, specs = c("diet", "drug")) |>
          contrast(method = "revpairwise", simple = "each", combine = TRUE, adjust = "none")
        p1_mat[i, 7] <- summary(lmm4_pairs)[1, "p.value"]
        p2_mat[i, 7] <- summary(lmm4_pairs)[2, "p.value"]
        p3_mat[i, 7] <- summary(lmm4_pairs)[3, "p.value"]
        p4_mat[i, 7] <- summary(lmm4_pairs)[4, "p.value"]
      }else{
        p1_mat[i, 7] <- NA
        p2_mat[i, 7] <- NA
        p3_mat[i, 7] <- NA
...
setnames(out_table,
         ...
         c("Contrast", "Cor Error", model_levels))
out_table |>
  kable(digits = c(1, 3, 3, 3, 3, 3, 3, 3, 3)) |>
  kable_styling() |>
...
big_table[, pless_rmaov.mul_contrast_1 := ifelse(rmaov.mul_contrast_1 < 0.05, 1, 0)]
big_table[, pless_rmaov.mul_contrast_3 := ifelse(rmaov.mul_contrast_3 < 0.05, 1, 0)]
...
m1 <- glm(pless_rmaov.mul_contrast_1 ~ r21,
          family = binomial,
          data = big_table[sim_id == 1,])
sim1_table[, rmaov.mul_contrast_1 := predict(m1, sim1_table,
                   type = "response")]
...
sim1_table[, aov1_contrast_1 := predict(m1, sim1_table,
                   type = "response")]
m1 <- glm(pless_rmaov.mul_contrast_3 ~ r32,
          family = binomial,
          data = big_table[sim_id == 1,])
sim1_table[, rmaov.mul_contrast_3 := predict(m1, sim1_table,
                   type = "response")]
...
sim1_table[, rmaov.mul_contrast_1_sim4 := predict(m1, sim1_table,
                   type = "response")]
m1 <- glm(pless_aov1_contrast_1 ~ r21,
          family = binomial,
          data = big_table[sim_id == 4,])
