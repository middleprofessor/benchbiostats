...
2. A RM-ANOVA with two groups is equivalent to a random intercept linear mixed model. A paired *t*-test is a special case of a linear mixed model.
...
```{r lmm-fig3-import-analysis, echo=FALSE, eval=FALSE}
file_name <- "41586_2019_985_MOESM9_ESM.xlsx"
file_path <- here(data_folder, data_from, file_name)
genotype_levels <- c("WT", "TKO")
fig3e_wide <- read_excel(file_path,
                         sheet = "Fig 3",
                         range = "G77:H83",
                         col_names = TRUE) |>
  data.table()
fig3e_wide[, experiment_id := paste("exp", .I)]
fig3e <- melt(fig3e_wide,
              id.vars = "experiment_id",
              measure.vars = genotype_levels,
              variable.name = "genotype",
              value.name = "pd1_mfi")
fig3e[, genotype := factor(genotype, levels = genotype_levels)]

lmm.ri <- lmer(pd1_mfi ~ genotype + (1 | experiment_id), data = fig3e)
lmm.ri_emm <- emmeans(lmm.ri, specs = "genotype")
lmm.ri_pairs <- contrast(lmm.ri_emm, method = "revpairwise") |>
  summary(infer = TRUE)
plot_response(lmm.ri, lmm.ri_emm, lmm.ri_pairs,
              join_blocks = TRUE,
              block_id = "experiment_id")
VarCorr(lmm.ri)
3396.6^2/(3396.6^2 + 1327^2)

rmaov.mul <- aov_4(pd1_mfi ~ genotype + (genotype | experiment_id), data = fig3e)
rmaov.mul_emm <- emmeans(rmaov.mul, specs = "genotype")
rmaov.mul_pairs <- contrast(rmaov.mul_emm, method = "revpairwise") |>
  summary(infer = TRUE)
...
lm.crd <- lm(pd1_mfi ~ genotype, data = fig3e)
summary(lm.crd)$sigma
sqrt(3396.6^2 + 1327^2)
```
...
7. Item 5 raises the question, why bother with random intercept model in a RCBD? The random intercept model (lmm.ri) should be used if there are covariates or if the response type (count, proportion, amount) and model check suggest a generalized linear model (glm).

### Plot the model

```{r lmm-fig2c_r_ggplot-auto, echo=FALSE, eval=FALSE}
