...
```{r}
#| echo: false
#| label: fig-modelcheck-histogram
#| fig-cap: "Histogram of the (A) response, showing two modes, one near the true mean of each group, and (B) residuals, with a mode for both groups at zero."
...
### A Normal Q-Q plot is used to check for characteristic departures from Normality {#sec-checking-normal-qq}
...
#### Normal QQ-plot of the fake data generated in @fig-modelcheck-histogram

```{r}
#| echo: false
#| label: fig-modelcheck-qq
...
m1 <- lm(Response ~ Treatment, data = fd)
fd[, Residual := residuals(m1)]
gg3_treatment <- ggcheck_the_qq(m1)
```
...
m1 <- lm(response ~ 1, data = fd_subset)
fd_subset[, residual := residuals(m1)]
gg3_normal <- ggcheck_the_qq(m1) +
  theme_pubr(base_size = 8) +
  ggtitle("Normal")
...
m1 <- lm(response ~ 1, data = fd_subset)
gg3_lognormal <- ggcheck_the_qq(m1) +
  theme_pubr(base_size = 8) +
  ggtitle("Lognormal")
...
gg2_negbin <- ggstripchart(data = fd_subset,
                           ...
                           alpha = 0.2,
                           palette = pal_okabe_ito) +
  scale_x_discrete(labels = c("Normal", "Negative Binomial")) +
  theme_pubr(base_size = 8) +
  theme(legend.position = "none")
...
gg2_t <- ggstripchart(data = fd_subset,
                      ...
                      alpha = 0.2,
                      palette = pal_okabe_ito) +
  scale_x_discrete(labels = c("Normal", "t distribution")) +
  theme_pubr(base_size = 8) +
  theme(legend.position = "none")
...
gg2_beta <- ggstripchart(data = fd_subset,
                         ...
                         alpha = 0.2,
                         palette = pal_okabe_ito) +
  scale_x_discrete(labels = c("Normal", "Beta")) +
  theme_pubr(base_size = 8) +
  theme(legend.position = "none")
...
```{r}
#| echo: false
#| label: fig-modelcheck-skewright
#| fig-cap: "Comparison of samples from a right skewed (Negative Binomial) and Normal distribution."
#| out.width: "90%"
#| fig.asp: 0.4

# , echo = FALSE, out.width="90%", fig.asp = .4
gg_right
...
```{r}
#| echo: false
#| label: fig-modelcheck-skewleft
#| fig-cap: "Comparison of samples from a left skewed (upper end of Beta) and Normal distribution."
#| out.width: "90%"
#| fig.asp: 0.4
...
```{r}
#| echo: false
#| label: fig-modelcheck-heavy
#| fig-cap: "Comparison of samples from a heavy tailed (t) and Normal distribution."
#| out.width: "90%"
#| fig.asp: 0.4
...
::: {.callout-important title="Mapping characteristic departures on a Q-Q plot to specific distributions"}
...
```{r}
#| label: modelcheck-fakedata-working
...
### ggcheck_the_qq from the ggplot_the_model source code
...
```{r}
#| label: modelcheck-working-ggchecktheqq
set.seed(1)
ggcheck_the_qq(m1)
```
...
```{r}
#| label: modelcheck-working-ggcheckthemodel
set.seed(1)
...
```{r}
#| label: modelcheck-working-car-default
...
```{r, eval=FALSE}
#| label: modelcheck-working-car-classic
...
### ggqqplot from the ggpubr package
...
```{r}
#| label: modelcheck-working-ggpubr
